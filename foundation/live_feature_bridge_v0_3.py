...
class SourceFact:
    ...
    status:Optional[str]=None; source_kind:str="MODEL_INPUT"; authority_rank:int=0
@dataclass(frozen=True)
...
def _validate_feature(name,v):
    ...
    if name in {"hr_rate","ob_rate","xb_rate","team_obp","b_rate","p_rate","rates_s","rates_d","rates_t","rates_hr","p_h","p_hr","rolling_league_rate"}:return prob(v)
    if name=="slot":return isinstance(v,int) and not isinstance(v,bool) and 1<=v<=9
    if name=="park":return isinstance(v,(int,float)) and not isinstance(v,bool) and math.isfinite(float(v)) and .25<=float(v)<=4
    if name=="own_bb":return isinstance(v,int) and not isinstance(v,bool) and v>=0
    if name=="own_bfp":return isinstance(v,int) and not isinstance(v,bool) and v>0
...
def resolve_fact(fact_key,sources,now,wager_cutoff,max_age_seconds):
    ...
    vals={_json(s.value) for s in valid}
    if len(vals)>1:return None,ResolutionFailure("CONFLICT",{"fact_key":fact_key,"conflicting":[{"source_id":s.source_id,"provider":s.provider,"value":s.value} for s in valid]})
...
def build_model_input(market,entity_id,game_id,fact_key_map,sources,now,wager_cutoff,freshness_limits,*,runtime_capabilities,schedule_attestation,known_entity_ids:Set[str]):
    ...
    resolved={};prov=[];lineup_status=None
    for feat in required:
        fk=fact_key_map[feat]
        if game_id not in fk:return None,ResolutionFailure("CROSS_GAME_FACT_KEY",{"feature":feat,"fact_key":fk,"game_id":game_id})
        chosen,fail=resolve_fact(fk,sources,now,wager_cutoff,freshness_limits.get(feat))
        ...
        resolved[feat]=chosen;prov.append(FeatureProvenance(feat,chosen.source_id,chosen.provider,chosen.event_time,chosen.retrieved_at,chosen.status))
    ...
    features={f:resolved[f].value for f in required}
    return ModelInput(market,entity_id,game_id,features,prov,lineup_status,_build_hash(market,entity_id,game_id,resolved,lineup_status,now,wager_cutoff)),None
