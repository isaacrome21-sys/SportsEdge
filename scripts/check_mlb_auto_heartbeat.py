...
def evaluate_runs(
    ...
    *,
    ...
    """Return a machine-readable fail-closed heartbeat assessment."""
    ...
    if not completed:
        return {
            "healthy": False,
            "problems": ["DEADMAN_NO_COMPLETED_AUTO_MLB_RUNS"],
...
def fetch_auto_mlb_runs(repo: str) -> list[dict[str, Any]]:
    ...
    # gh api switches to POST when -f fields are supplied unless GET is explicit.
    # This endpoint is read-only, so pin the method to prevent a false API error.
