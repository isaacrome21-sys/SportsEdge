...
def _prop_lane_state(sport: str) -> tuple[bool, str | None]:
    """Return whether a sport has an executable prop engine under the authority file."""
...
def main() -> int:
    ...
    if prop_executable:
        ...
        prop_returncode: int | None = int(prop.returncode)
        ...
        prop_rows = _blocked_row("PLAYER_PROPS", blocker)
        prop_payload = {
            "status": "NO_ENGINE",
            "blocker": blocker,
            "sport": sport,
    ...
    if game.returncode != 0:
        status = "BLOCKED"
    elif prop_executable and prop_returncode == 0:
        status = "SUCCESS"
    else:
        status = "PARTIAL"
