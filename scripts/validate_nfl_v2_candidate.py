#!/usr/bin/env python3
"""Compare an isolated NFL V2 candidate against the frozen key-number tolerance.

This is diagnostic evidence only. It intentionally does not emit the canonical
production math attestation contract and cannot promote or deploy a candidate.
"""
...
_CANDIDATES = {
    NFL_M2_V2_CANDIDATE_MODEL_ID: {
        ...
        "profile_contract": "NFL_M2_V2_CANDIDATE_OOS_SIGNED_KEY_PMF_V1",
        "profile_version": "nfl-m2-v2a-candidate-oos-key-emergent-v1",
    },
    NFL_M2_V2B_CANDIDATE_MODEL_ID: {
        "distribution_contract": NFL_M2_V2B_DISTRIBUTION_CONTRACT,
        "profile_contract": "NFL_M2_V2B_CANDIDATE_OOS_SIGNED_KEY_PMF_V1",
        "profile_version": "nfl-m2-v2b-candidate-oos-key-emergent-v1",
    },
    NFL_M2_V2C_CANDIDATE_MODEL_ID: {
        "distribution_contract": NFL_M2_V2C_DISTRIBUTION_CONTRACT,
        "profile_contract": "NFL_M2_V2C_OOS_SIGNED_KEY_PMF_V1",
        "profile_version": "nfl-m2-v2c-candidate-oos-key-emergent-v1",
    },
}
...
def main() -> int:
    ...
    contract = _CANDIDATES.get(model_id)
    if contract is None:
        raise SystemExit("NFL_M2_V2_MODEL_ID_UNSUPPORTED")
    if evidence.get("distribution_contract") != contract["distribution_contract"]:
    ...
    if profile_payload.get("contract") != contract["profile_contract"]:
        raise SystemExit("NFL_M2_V2_CANDIDATE_PROFILE_CONTRACT_INVALID")
    if profile_payload.get("model_id") != model_id:
    ...
    profile = build_nfl_simulator_profile(audit, version=contract["profile_version"])
    ...
    payload = {
        ...
        "model_id": model_id,
        "distribution_contract": contract["distribution_contract"],
