...
def _score_all(
    *, scorer_rows: list[dict[str, str]], markets: list[str], policy: Mapping[str, Any],
    policy_path: Path, catalog_path: Path, scorer_csv: Path,
) -> dict[str, Any]:
    ...
    return {
        ...
        "markets": {
            market: score(by_market[market], unauthorized=market in blocked_nway)
...
def _matrix(
    markets: list[str], metrics: Mapping[str, Any], replay: Mapping[str, Any],
) -> dict[str, Any]:
    exclusions_by_market: dict[str, int] = {market: 0 for market in markets}
    for exclusion in replay.get("exclusions", []):
        # The replay bridge may fail before a market is trustworthy enough to retain;
        # keep unknown-market exclusions at bundle level rather than guessing a market.
...
def main() -> int:
    ...
    matrix = _matrix(markets, metrics, replay)
