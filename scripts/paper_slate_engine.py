#!/usr/bin/env python3
"""PAPER slate: recency team ratings from public results + DraftKings prices.

This is not the frozen CFB ridge or NFL attempt-9 CI artifact.
It is a market-blind scoring model for PAPER cards only.
"""
...
def nfl_ratings() -> dict[str, dict[str, float]]:
    ...
    for row in rows:
        ...
        by_team[str(home)].append((day, hs, aws))
        by_team[str(away)].append((day, aws, hs))
...
def espn_cfb_ratings(days: int = 28) -> dict[str, dict[str, float]]:
    ...
    for offset in range(days):
        ...
        url = ESPN_CFB.format(date=day.strftime("%Y%m%d"))
        try:
        ...
        for event in payload.get("events") or []:
            ...
            try:
                scored = [
                    (str(p["team"]["displayName"]), float(p["score"]))
                    for p in parts
                ]
            except (KeyError, TypeError, ValueError):
                continue
