...
def fetch_once() -> tuple[RawDraftKingsBoard, str | None]:
    ...
    try:
        ...
        raise Blocked("HTTP_ERROR", f"status={exc.code}") from exc
...
def missed_path(out_dir: Path, week: int, kind: str, stamp: str) -> Path:
    if kind == "OPENER":
        return out_dir / f"week{week:02d}" / "opener_missed.json"
    return out_dir / f"week{week:02d}" / "final" / f"{stamp}_missed.json"


def success_path(out_dir: Path, week: int, kind: str, stamp: str) -> Path:
    if kind == "OPENER":
        return out_dir / f"week{week:02d}" / "opener.json"
    return out_dir / f"week{week:02d}" / "final" / f"{stamp}.json"
...
def run(*, force: bool, as_of: datetime | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        write_missed(missed_path(out_dir, week, "OPENER" if force else "FINAL", iso_z(now)), exc.reason, exc.detail, "FETCH", week, now)
        report.update(status="MISSED_OR_BLOCKED", reason=exc.reason, detail=exc.detail)
        return report
    ...
    for row in paired:
        ...
        for kind in kinds:
            buckets.setdefault((stamp, week, kind), []).append(row)

    written = []
    for (stamp, week, kind), games in buckets.items():
        path = success_path(out_dir, week, kind, stamp)
        record = {
            "authority_footer": AUTHORITY,
            "book": "draftkings",
            "capture_kind": kind,
    ...
    if not written and not force:
        report.update(status="NO_WINDOW", skipped_outside_window=skipped_window, raw_sha256=raw_sha, paired=len(paired))
        return report
    if not written:
        week = week_of(now, cfg)
        write_missed(missed_path(out_dir, week, "FINAL", iso_z(now)), "NO_PAIRED_IN_WINDOW", "", "FINAL", week, now)
        report.update(status="MISSED_OR_BLOCKED", reason="NO_PAIRED_IN_WINDOW", raw_sha256=raw_sha)
        return report
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--force", action="store_true", help="write FINAL rows even outside frozen windows (proof run)")
    ...
    return 0 if report.get("status") in {"CAPTURED", "NO_WINDOW"} else 2
