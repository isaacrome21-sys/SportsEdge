#!/usr/bin/env python3
"""NFL Attempt 8: point-in-time turnover-adjusted efficiency."""
...
def stats(seasons):
    ...
    for s in sorted(seasons):
        ...
        f=pd.read_parquet(io.BytesIO(raw),columns=["game_id","posteam","defteam","play_type","season_type","interception","fumble_lost"])
        ...
        for (gid,off,defn),g in f.groupby(["game_id","posteam","defteam"],sort=False):
            n=len(g);give=float(g["interception"].fillna(0).eq(1).sum()+g["fumble_lost"].fillna(0).eq(1).sum())/max(1,n)
            out[(str(gid),str(off))]={"giveaway_rate":give,"takeaway_rate":give,"defteam":str(defn)}
...
def features(games,st):
    ...
    names=["home_points_for","home_points_against","away_points_for","away_points_against","home_net","away_net","home_giveaway_rate","home_takeaway_rate","away_giveaway_rate","away_takeaway_rate","home_net_turnover_rate","away_net_turnover_rate"]
    for d,grp in groupby(ordered,key=lambda g:g["date"]):
        ...
        for g in batch:
            ...
            X.append([hp[0],hp[1],ap[0],ap[1],hp[0]-hp[1],ap[0]-ap[1],hp[2],hp[3],ap[2],ap[3],hp[3]-hp[2],ap[3]-ap[2]])
            ym.append(g["hs"]-g["as"]);yt.append(g["hs"]+g["as"]);dates.append(d);keys.append((d,str(g["id"]),g))
        for g in batch:
            h,a=g["home"],g["away"];hs=st.get((str(g["id"]),str(h)));aa=st.get((str(g["id"]),str(a)))
            if hs and aa:
                hist[h].append([g["hs"],g["as"],hs["giveaway_rate"],aa["takeaway_rate"]])
                hist[a].append([g["as"],g["hs"],aa["giveaway_rate"],hs["takeaway_rate"]])
...
def bench(rep,games,keys,start,end,target):
    ...
    if len(pred)!=len(sel):raise RuntimeError(f"TURNOVER_ALIGNMENT_FAILED:{target}:{len(pred)}:{len(sel)}")
    ...
    return r
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--seasons",default="2010,2011,2012,2013,2014,2015,2016,2017,2018,2019");ap.add_argument("--policy",default="config/nfl_research_search_policy_v1.json");ap.add_argument("--out",default="artifacts/football_baselines_attempt8.json");a=ap.parse_args()
    ...
    if len(X)<200:raise RuntimeError(f"TURNOVER_INSUFFICIENT_FEATURE_ROWS:{len(X)}")
    ...
    report={"schema":"NFL_TURNOVER_ATTEMPT8_V1","source":URL,"games_source_sha256":gsha,"pbp_source_sha256":ss,"policy_path":str(pp),"policy_sha256":hashlib.sha256(pp.read_bytes()).hexdigest(),"training_years":[ts,te],"holdout_years":[start,end],"feature_set":"turnover_adjusted_plus_baseline","feature_names":names,"games_fetched":len(games),"usable_rows":len(X),"status":"RESEARCH_ONLY_NOT_MODEL_P","targets":targets,"control_baseline_same_holdout":control,"closing_line_benchmark":{"margin":bench(targets["margin"],games,keys,start,end,"margin"),"total":bench(targets["total"],games,keys,start,end,"total")}}
