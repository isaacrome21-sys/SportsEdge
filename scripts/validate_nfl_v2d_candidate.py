#!/usr/bin/env python3
"""Compare NFL V2D OOS signed key mass against the frozen historical target."""
...
_PROFILE_CONTRACT = "NFL_M2_V2D_OOS_SIGNED_KEY_PMF_V1"
_PROFILE_VERSION = "nfl-m2-v2d-candidate-oos-key-emergent-v1"
...
def main() -> int:
    ...
    if evidence.get("status") != "DIAGNOSTIC_CANDIDATE_ONLY" or evidence.get("promotion_eligible") is not False:
        raise SystemExit("NFL_M2_V2D_DIAGNOSTIC_STATUS_REQUIRED")
    if evidence.get("model_id") != NFL_M2_V2D_CANDIDATE_MODEL_ID:
        raise SystemExit("NFL_M2_V2D_MODEL_ID_MISMATCH")
    if evidence.get("distribution_contract") != NFL_M2_V2D_DISTRIBUTION_CONTRACT:
        raise SystemExit("NFL_M2_V2D_DISTRIBUTION_CONTRACT_MISMATCH")
    ...
    if not isinstance(profile_payload, dict) or profile_payload.get("contract") != _PROFILE_CONTRACT:
        raise SystemExit("NFL_M2_V2D_PROFILE_CONTRACT_INVALID")
    if profile_payload.get("model_id") != NFL_M2_V2D_CANDIDATE_MODEL_ID:
        raise SystemExit("NFL_M2_V2D_PROFILE_IDENTITY_MISMATCH")
    raw_pmf = profile_payload.get("signed_key_probability")
    if not isinstance(raw_pmf, dict):
        raise SystemExit("NFL_M2_V2D_PMF_REQUIRED")

    profile = build_nfl_simulator_profile(audit, version=_PROFILE_VERSION)
    ...
    try:
        ...
        raise SystemExit("NFL_M2_V2D_PMF_INVALID") from exc
    if any(value < 0.0 or value > 1.0 for value in candidate_pmf.values()):
        raise SystemExit("NFL_M2_V2D_PMF_INVALID")
