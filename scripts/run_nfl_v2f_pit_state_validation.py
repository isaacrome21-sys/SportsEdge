#!/usr/bin/env python3
"""Run the preregistered NFL V2F PIT-state diagnostic on frozen production sources."""
...
from sportsedge.sports.nfl.m2_v2f_pit_state import (
    NFL_M2_V2F_PIT_STATE_CONTRACT,
    build_nfl_m2_v2f_pit_state_rows,
)

_GIT_SHA_RE = re.compile(r"^[0-9a-f]{40}$")
_EXPERIMENT_ID = "nfl_m2_v2f_pit_state_blend_v1"


def _assert_source(path: Path, expected: str, label: str) -> None:
    if not path.is_file():
        raise SystemExit(f"NFL_M2_V2F_SOURCE_FILE_MISSING:{label}")
    if _sha(path) != expected:
        raise SystemExit(f"NFL_M2_V2F_SOURCE_FILE_HASH_MISMATCH:{label}")
...
def main() -> int:
    ...
    p.add_argument("--out", type=Path, default=Path("artifacts/football/nfl_m2_v2f_pit_state_validation.json"))
    args = p.parse_args()

    git_sha = str(args.git_sha).strip().lower()
    if not _GIT_SHA_RE.fullmatch(git_sha):
        raise SystemExit("NFL_M2_V2F_GIT_SHA_INVALID")
    if args.end_season < args.start_season or args.start_season < 2016:
        raise SystemExit("NFL_M2_V2F_SEASON_RANGE_INVALID")

    try:
        manifest_payload = json.loads(args.source_manifest.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise SystemExit(f"NFL_M2_V2F_SOURCE_MANIFEST_INVALID:{exc}") from exc
    ...
    for name, path in required.items():
        ...
        if expected is None:
            raise SystemExit(f"NFL_M2_V2F_SOURCE_MANIFEST_ENTRY_MISSING:{name}")
    ...
    try:
        ...
        raise SystemExit(f"NFL_M2_V2F_STARTER_OVERRIDE_INVALID:{exc}") from exc
    ...
    if after:
        raise SystemExit(f"NFL_M2_V2F_STARTING_QB_COVERAGE_GAPS:{len(after)}")

    history_rows = build_nfl_m2_v2f_pit_state_rows(
        schedule,
        pbp,
        participation,
        depth,
    ...
    if not history_rows:
        raise SystemExit("NFL_M2_V2F_HISTORY_ROWS_EMPTY")
    ...
    evidence = build_nfl_m2_v2d_candidate_evidence(
        ...
        total_kernel_scale=1.0,
    )
    downstream_model_id = evidence["model_id"]
    evidence.update({
        "experiment_id": _EXPERIMENT_ID,
        "history_feature_variant": NFL_M2_V2F_PIT_STATE_CONTRACT,
        "downstream_probability_model_id": downstream_model_id,
    ...
    print(json.dumps({
        "status": "V2F_PIT_STATE_DIAGNOSTIC_COMPLETE",
