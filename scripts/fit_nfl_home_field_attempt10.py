#!/usr/bin/env python3
"""NFL Attempt 10: point-in-time team-specific home-field effects."""
...
def features(games):
    ordered=sorted(games,key=lambda g:(g["date"],g["id"]));hist=defaultdict(list);home_hist=defaultdict(list);away_hist=defaultdict(list);X=[];ym=[];yt=[];dates=[];keys=[]
    names=["home_points_for","home_points_against","away_points_for","away_points_against","home_net","away_net","home_home_margin","away_away_margin"]
    for d,grp in groupby(ordered,key=lambda g:g["date"]):
        batch=list(grp)
        for g in batch:
            h,a=g["home"],g["away"]
            if len(hist[h])<5 or len(hist[a])<5 or not home_hist[h] or not away_hist[a]:continue
            hp=np.mean(np.asarray(hist[h][-10:],float),axis=0);ap=np.mean(np.asarray(hist[a][-10:],float),axis=0)
            X.append([hp[0],hp[1],ap[0],ap[1],hp[0]-hp[1],ap[0]-ap[1],float(np.mean(home_hist[h][-10:])),float(np.mean(away_hist[a][-10:]))])
            ym.append(g["hs"]-g["as"]);yt.append(g["hs"]+g["as"]);dates.append(d);keys.append((d,str(g["id"]),g))
        for g in batch:
            h,a=g["home"],g["away"];hist[h].append((g["hs"],g["as"]));hist[a].append((g["as"],g["hs"]));home_hist[h].append(g["hs"]-g["as"]);away_hist[a].append(g["as"]-g["hs"])
...
def bench(rep,games,keys,start,end,target):
    ...
    if len(pred)!=len(sel):raise RuntimeError(f"HOME_FIELD_ALIGNMENT_FAILED:{target}:{len(pred)}:{len(sel)}")
    ...
    return r
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--seasons",default="2010,2011,2012,2013,2014,2015,2016,2017,2018,2019");ap.add_argument("--policy",default="config/nfl_research_search_policy_v1.json");ap.add_argument("--out",default="artifacts/football_baselines_attempt10.json");a=ap.parse_args()
    ...
    if len(X)<200:raise RuntimeError(f"HOME_FIELD_INSUFFICIENT_FEATURE_ROWS:{len(X)}")
