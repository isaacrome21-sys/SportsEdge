...
def _write_shadow_ledger(*, report: dict, source: dict, root: Path) -> dict | None:
    rows = [
        dict(row) for row in report.get("results", [])
        if row.get("model_p") is not None and row.get("shadow_status") in {
            "SHADOW_BET", "SHADOW_PASS", "MODEL_CANDIDATE_ONE_SIDED"
        }
    ]
    if not rows:
        return None
    ...
    try:
        ...
        created = False
    return {"schema_version": LEDGER_SCHEMA, "path": str(target), "sha256": digest,
            "row_count": len(rows), "created": created}


def run(*, now: datetime, output: Path, raw_root: Path, ledger_root: Path,
        require_confirmed_lineup: bool = True) -> tuple[dict, int]:
        ...
        if not snapshot.quotes:
            payload = {
                ...
                "report": {
                    ...
                    "run_status": "BLOCKED_NO_DIRECT_DK_QUOTES",
                    "results": [],
                    "source_failures": [*source["failures"], {"reason": "DIRECT_DK_NO_ADMISSIBLE_QUOTES"}],
                    "summary": {"quote_count": 0, "model_priced": 0, "official_bets": 0},
                },
                "direct_dk_source": source,
                "governance": {"official_authority": False, "promotion_authority": False,
                               "deployment_eligibility_changed": False},
            }
            rc = 2
        else:
            machine = run_it_mlb(
                mode="HYBRID", quotes=list(snapshot.quotes), now=current,
...
def main() -> int:
    ...
    parser.add_argument("--output", default="artifacts/run_it/mlb_card.json")
    ...
    payload, rc = run(
        now=datetime.now(timezone.utc), output=Path(args.output), raw_root=Path(args.raw_dir),
        ledger_root=Path(args.shadow_ledger_dir), require_confirmed_lineup=args.require_confirmed_lineup,
    )
