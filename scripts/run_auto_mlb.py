...
def main()->int:
    ...
    now=datetime.now(timezone.utc); infrastructure_blocked=False
    try:
        ...
        # Native odds is the canonical Full Model path because it can acquire all
        # game and supported player markets under one identity contract.
        if odds_api_keys:
            if not game_score_path or not nrfi_path: raise AutoRunnerError("GAME_ARTIFACT_CONFIG_MISSING")
            ...
            if not features: raise AutoRunnerError("FEATURE_PROVIDER_CONFIG_MISSING")
            report=run_auto_mlb(quote_url=quotes,feature_url=features,**common)
        else: raise AutoRunnerError("QUOTE_PROVIDER_CONFIG_MISSING")
        payload=report_to_dict(report); payload["bettor_card"]=build_bettor_card(report.results,min_edge=args.min_edge)
    except Exception as exc:
        infrastructure_blocked=True
        payload={"slate_date_ct":now.astimezone(CHICAGO_TZ).date().isoformat(),"generated_at_utc":now.isoformat(),"run_status":"BLOCKED","results":[],"source_failures":[{"reason":f"{type(exc).__name__}: {exc}"}],"bettor_card":build_bettor_card([],min_edge=args.min_edge)}
