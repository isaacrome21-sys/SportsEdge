#!/usr/bin/env python3
"""Run isolated NFL V2D diagnostic on the exact production-history inputs."""
...
def _verified_manifest(payload: dict) -> tuple[str, dict[str, str]]:
    ...
    if len(expected) != 64:
        raise SystemExit("NFL_M2_V2D_SOURCE_MANIFEST_HASH_MISSING")
    core = {
    ...
    if manifest_sha256(core) != expected:
        raise SystemExit("NFL_M2_V2D_SOURCE_MANIFEST_HASH_MISMATCH")
    sources = payload.get("sources")
    if not isinstance(sources, list):
        raise SystemExit("NFL_M2_V2D_SOURCE_MANIFEST_ROWS_INVALID")
    by_name: dict[str, str] = {}
    for raw in sources:
        if not isinstance(raw, dict):
            raise SystemExit("NFL_M2_V2D_SOURCE_MANIFEST_ROW_INVALID")
