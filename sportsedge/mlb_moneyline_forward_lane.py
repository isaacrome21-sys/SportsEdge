...
from .edge_floors import load_edge_floor_config, require_frozen_edge_floor
...
def load_forward_lane_binding(
    ...
    *,
    ...
    for value, name in ((pred, "prediction window"), (decision, "decision capture"), (paper, "paper decision"), (close, "close capture"), (authority, "authority")):
    ...
    if float(decision.get("target_minutes_before_start", -1)) != 30.0 or float(decision.get("early_tolerance_minutes", -1)) != 6.0:
        raise MLBMoneylineForwardLaneError("decision window mismatch")
    if decision.get("direction") != "EARLY_ONLY_AT_OR_BEFORE_TARGET" or decision.get("two_sided_required") is not True or decision.get("must_be_before_start") is not True or decision.get("lock_once_recorded") is not True:
        raise MLBMoneylineForwardLaneError("decision capture contract mismatch")
    if paper.get("state") != "PAPER" or float(paper.get("stake_units", -1)) != 0.0 or paper.get("require_positive_ev") is not True:
        raise MLBMoneylineForwardLaneError("paper state contract mismatch")
    if close.get("selection") != "LAST_VERIFIABLE_PAIRED_PRE_FIRST_PITCH_QUOTE" or close.get("two_sided_required") is not True or close.get("must_be_before_start") is not True:
        raise MLBMoneylineForwardLaneError("close capture contract mismatch")
    if any(authority.get(key) is not False for key in ("promotion_authority", "deployment_change_allowed", "staking_change_allowed", "official_change_allowed")):
    ...
    floor = require_frozen_edge_floor(market=str(paper.get("edge_floor_market")), sport="MLB", config=floor_cfg)
...
def require_record_binding(record: Mapping[str, Any], binding: Mapping[str, Any] | None = None) -> None:
