...
ENGINE_VERSION = "mlb_pitcher_joint_empirical_v1"
...
def _price_values(values: Sequence[int], line: float, side: str) -> tuple[float, float]:
    ...
    if abs(p_over + p_under + p_push - 1.0) > 1e-12:
        raise PitcherJointEngineError("probability mass does not conserve")
...
def price_pitcher_market(model_input: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if market.startswith("EITHER_PITCHER_"):
        ...
        vals_a = [_value(r, base) for r in pool_a]
        vals_b = [_value(r, base) for r in pool_b]
        # Empirical product measure over independent prior-start draws. The market
        # event is "either pitcher" satisfying the quoted side proposition.
        states = [(a, b) for a in vals_a for b in vals_b]
        if side == "OVER":
            wins = sum((a > line) or (b > line) for a, b in states)
            pushes = sum(not ((a > line) or (b > line)) and ((a == line) or (b == line)) for a, b in states) if float(line).is_integer() else 0
        else:
            wins = sum((a < line) or (b < line) for a, b in states)
            pushes = sum(not ((a < line) or (b < line)) and ((a == line) or (b == line)) for a, b in states) if float(line).is_integer() else 0
        p = wins / len(states)
        p_push = pushes / len(states)
        identity_features: Any = {"pitcher_a_history": pool_a, "pitcher_b_history": pool_b}
    else:
        pool = _normalize_pool(features.get("history_pool"), "history_pool")
        values = [_value(r, market) for r in pool]
        p, p_push = _price_values(values, line, side)
        identity_features = {"history_pool": pool}
    ...
    return {
        ...
        "line": line, "side": side, "model_p": float(p), "push_p": float(p_push),
        "model_input_hash": digest, "engine_version": ENGINE_VERSION,
        "seed_policy": "analytic_empirical_joint_start_rows", "mc_paths": 0,
    }
