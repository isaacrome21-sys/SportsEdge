...
The live card remains a mutable latest-view artifact. This journal is the append-only
...
def _modeled_prediction(row: Mapping[str, Any], index: int) -> dict[str, Any] | None:
    ...
    if status not in DECISION_STATUSES:
        raise PredictionJournalError(
            f"result[{index}] modeled row lacks BET/PASS decision: {status or 'MISSING'}"
        )
    ...
    if market in STAGE1_GAME_MARKETS:
        ...
        out["distribution_sha256"] = _valid_sha256(
...
def build_prediction_journal_record(payload: Mapping[str, Any]) -> dict[str, Any] | None:
    """Build one immutable journal record, or None when no modeled rows exist."""
    ...
    source_report_raw = _canonical_bytes(dict(payload))
    source_report_sha256 = _sha256_bytes(source_report_raw)
...
def write_prediction_journal(
    payload: Mapping[str, Any],
    *,
    root: str | Path = "artifacts/prediction_journal",
) -> JournalWriteResult | None:
    """Create an immutable journal file; exact duplicate writes are idempotent.

    The file is opened in exclusive-create mode. Existing content is never replaced.
    A repeated identical run resolves to the same content-addressed path and is
    accepted only if the bytes already on disk are exactly identical.
    """
