...
# Verified The Odds API baseball first-inning total market key. This market is
# acquired from the event-level odds endpoint, not the featured bulk path.
...
def parse_first_inning_odds(
    payload: Mapping[str, Any], *, game: GameSnapshot,
    market_key: str = FIRST_INNING_TOTAL_MARKET_KEY,
    ttl_seconds: int = DEFAULT_TTL_SECONDS,
) -> GameOddsSnapshot:
    """Parse first-inning total-runs 0.5 into YRFI/NRFI quotes."""
    if not isinstance(market_key, str) or not market_key.strip():
        raise OddsApiSourceError("FIRST_INNING_MARKET_KEY_REQUIRED")
        ...
        for market in bookmaker.get("markets") or []:
            ...
            for outcome in market.get("outcomes") or []:
                try:
                    ...
                    if float(point) != 0.5:
                        raise OddsApiSourceError("FIRST_INNING_LINE_NOT_HALF_RUN")
                    ...
                    base = {
                        "game_id": str(game.game_pk), "period": "1ST", "book_key": book_key,
                        "sportsbook": book_title, "retrieved_at": retrieved_at, "ttl_seconds": ttl_seconds,
                        "american_odds": price, "raw_market_name": mk,
                        "away_team": str(game.away_name), "home_team": str(game.home_name),
                    }
                    quotes.append({**base, "market": canonical_market, "side": raw_side,
                                   "selection": canonical_market, "line": 0.5})
                except Exception as exc:
                    failures.append({"reason": str(exc), "game_id": str(game.game_pk), "book_key": book_key, "raw_market_name": mk})
...
def fetch_mlb_game_quotes(*, api_key: str, schedule: Iterable[GameSnapshot], opener: Callable = urlopen, bookmakers: Iterable[str] = DEFAULT_BOOKMAKERS, ttl_seconds: int = DEFAULT_TTL_SECONDS) -> GameOddsSnapshot:
    ...
    url = _event_url(
        "/sports/baseball_mlb/odds", api_key=api_key,
...
def fetch_event_first_inning_quotes(*, api_key: str, provider_event_id: str, game: GameSnapshot, opener: Callable = urlopen, bookmakers: Iterable[str] = DEFAULT_BOOKMAKERS, ttl_seconds: int = DEFAULT_TTL_SECONDS) -> GameOddsSnapshot:
    ...
    if not eid:
        raise OddsApiSourceError("ODDS_PROVIDER_EVENT_ID_MISSING")
    url = _event_url(
        f"/sports/baseball_mlb/events/{eid}/odds", api_key=api_key,
        params={"regions":"us","bookmakers":requested_books,"markets":FIRST_INNING_TOTAL_MARKET_KEY,"oddsFormat":"american","dateFormat":"iso","includeSids":"true"},
    )
    payload = _get_json(url, opener=opener, label="first-inning-market")
    if not isinstance(payload, Mapping):
        raise OddsApiSourceError("ODDS_FIRST_INNING_RESPONSE_NOT_OBJECT")
...
def fetch_event_market_keys(*, api_key: str, provider_event_id: str, opener: Callable = urlopen) -> set[str]:
    ...
    if isinstance(payload, Mapping):
        ...
        raise OddsApiSourceError("ODDS_EVENT_MARKETS_RESPONSE_MALFORMED")
