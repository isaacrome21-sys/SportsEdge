...
COORDINATE_FALLBACKS = {
    5340: {
        ...
        "url": "https://www.wikidata.org/wiki/Special:EntityData/Q60830511.json",
    }
}
...
def venue_ids_from_schedule_payloads(payloads: Iterable[Any], *, game_types: frozenset[str] = MODELED_GAME_TYPES) -> list[int]:
    """Discover venue IDs while tolerating only identity-consistent duplicate game rows."""
    ids: set[int] = set()
...
def _mlb_timezone(payload: Any, expected_venue_id: int) -> str:
    if not isinstance(payload, dict):
        raise MLBV7VenueReferenceError("VENUE_PAYLOAD_INVALID")
    venues = payload.get("venues")
    if not isinstance(venues, list) or len(venues) != 1 or not isinstance(venues[0], dict):
        raise MLBV7VenueReferenceError("VENUE_RESPONSE_NOT_SINGLETON")
    venue = venues[0]
    try:
        venue_id = int(venue["id"])
    except Exception as exc:
        raise MLBV7VenueReferenceError("VENUE_ID_MISSING") from exc
    if venue_id != int(expected_venue_id):
        raise MLBV7VenueReferenceError("VENUE_ID_MISMATCH")
    ...
    if not isinstance(tz_id, str) or not tz_id or "/" not in tz_id:
        raise MLBV7VenueReferenceError("VENUE_TIMEZONE_MISSING")
    return tz_id


def parse_wikidata_coordinate(payload: Any, *, entity_id: str, expected_label: str) -> tuple[float, float]:
    if not isinstance(payload, dict):
        raise MLBV7VenueReferenceError("WIKIDATA_PAYLOAD_INVALID")
    entities = payload.get("entities")
    if not isinstance(entities, dict) or entity_id not in entities or not isinstance(entities[entity_id], dict):
        raise MLBV7VenueReferenceError("WIKIDATA_ENTITY_MISSING")
    ...
    if label != expected_label:
        raise MLBV7VenueReferenceError("WIKIDATA_ENTITY_LABEL_MISMATCH")
    claims = entity.get("claims") or {}
    p625 = claims.get("P625") if isinstance(claims, dict) else None
    if not isinstance(p625, list) or not p625:
        raise MLBV7VenueReferenceError("WIKIDATA_COORDINATE_MISSING")
    ...
    if len(unique) != 1:
        raise MLBV7VenueReferenceError("WIKIDATA_COORDINATE_AMBIGUOUS" if unique else "WIKIDATA_COORDINATE_MISSING")
    return unique[0]
...
def build_venue_reference_rows(
    ...
    *,
    ...
    fetch_coordinate_fallback: Callable[[int, dict[str, str]], Any] | None = None,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    rows, failures = [], []
    for venue_id in sorted(set(int(v) for v in venue_ids)):
        payload = fetch_venue_payload(venue_id)
        try:
            parsed = parse_venue_reference(payload, venue_id)
            row = asdict(parsed)
            row["coordinate_source"] = "MLB_STATSAPI"
            row["timezone_source"] = "MLB_STATSAPI"
            ...
            if str(exc) != "VENUE_COORDINATES_MISSING" or fallback is None or fetch_coordinate_fallback is None:
                ...
                continue
            try:
                tz = _mlb_timezone(payload, venue_id)
                secondary = fetch_coordinate_fallback(venue_id, fallback)
                latitude, longitude = parse_wikidata_coordinate(
                    secondary,
                    entity_id=fallback["entity_id"],
                    expected_label=fallback["expected_label"],
                )
                row = {
                    "venue_id": venue_id,
                    "latitude": latitude,
                    "longitude": longitude,
                    "timezone": tz,
                    "coordinate_source": f"WIKIDATA_{fallback['entity_id']}",
                    "timezone_source": "MLB_STATSAPI",
                }
            except MLBV7VenueReferenceError as secondary_exc:
                failures.append({"venue_id": venue_id, "reason": str(secondary_exc)})
                continue
...
def build_venue_reference_report(*, coverage_start: str, coverage_end: str, venue_ids: list[int], rows: list[dict[str, Any]], failures: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "secondary_coordinate_source_count": len(fallback_rows),
        "secondary_coordinate_venue_ids": sorted(int(row["venue_id"]) for row in fallback_rows),
...
def build_attestation(*, coverage_start: str, coverage_end: str, evidence_files: list[dict[str, str]]) -> dict[str, Any]:
    if not evidence_files:
        raise MLBV7VenueReferenceError("EVIDENCE_FILES_REQUIRED")
    for item in evidence_files:
        if not isinstance(item, dict) or not isinstance(item.get("path"), str) or not isinstance(item.get("sha256"), str) or len(item["sha256"]) != 64:
            raise MLBV7VenueReferenceError("EVIDENCE_FILE_INVALID")
    return {
        "source_class": SOURCE_CLASS,
        "coverage_start": coverage_start,
        "coverage_end": coverage_end,
