...
def _latest_confirmed_final_snapshot(
    ...
    """Return history ending at the latest timecode whose full snapshot proves Final."""
    if not timecodes:
        raise MLBV7TravelHistoryError("FINAL_GAME_TIMESTAMPS_MISSING")
    for idx in range(len(timecodes) - 1, -1, -1):
        timecode = timecodes[idx]
        # Use the full historical payload. Hosted evidence showed a terminal MLB
        # timecode can remain Live even though an earlier historical snapshot is
        # already Final; rest/travel may only use a timestamp that itself proves Final.
