...
def _convert(result) -> UnifiedCardResult:
    ...
    odds=getattr(result,"american_odds",None)
    scored=None
    if model_p is not None and odds is not None:
        scored=opportunity_score(model_p=model_p,book_american_odds=odds)
    return UnifiedCardResult(
        game_id=result.game_id, market=result.market, entity_id=result.entity_id,
        line=result.line, side=result.side, american_odds=odds, model_p=model_p,
        bet_status=result.bet_status, reason=result.reason,
        ...
        fair_american_odds=scored.fair_american_odds if scored else None,
        sportsedge_score=scored.score if scored else None,
        stars=scored.stars if scored else None,
        opportunity_edge_probability_points=scored.edge_probability_points if scored else None,
        ...
        mc_paths=getattr(result,"mc_paths",None), book_key=getattr(result,"book_key",None),
...
def _assert_ledger_precondition(result: UnifiedCardResult) -> None:
    ...
    if modeled != decided:
        raise RuntimeError("priced decision ledger invariant violated: "+f"market={result.market} status={result.bet_status} model_p={result.model_p}")

def run_unified_card(*, games:list[LiveGame], feature_rows:list[Mapping[str,Any]], quotes:list[Mapping[str,Any]],
    ingestion_now:datetime, finalization_now:datetime, registry_path:str="config/deployments.json",
    require_confirmed_lineup:bool=True, edge_floor_config_path:str=DEFAULT_EDGE_FLOOR_CONFIG,
    kelly_multiplier:float=.25)->list[UnifiedCardResult]:
    if require_confirmed_lineup is not True: raise ValueError("CONFIRMED_LINEUP_REQUIRED_FOR_PRODUCTION")
    indexed=[]; output={}
    for i,raw_quote in enumerate(quotes):
        try:
            quote=validate_canonical_quote(raw_quote); market=str(quote["market"])
            if market not in SUPPORTED_MARKETS: raise ValueError(f"unsupported unified market: {market}")
    ...
    if indexed:
        results=run_generic_card(games=games,feature_rows=feature_rows,quotes=[q for _,q in indexed],
            ingestion_now=ingestion_now,finalization_now=finalization_now,registry_path=registry_path,
            require_confirmed_lineup=True,edge_floor_config_path=edge_floor_config_path,kelly_multiplier=kelly_multiplier)
        if len(results)!=len(indexed): raise RuntimeError("canonical pipeline changed quote cardinality")
        for (i,_),result in zip(indexed,results):
            converted=_convert(result); _assert_ledger_precondition(converted); output[i]=converted
    if len(output)!=len(quotes): raise RuntimeError("unified runner failed to preserve quote cardinality")
