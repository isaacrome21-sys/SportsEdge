...
def run_auto_mlb_native_odds(*,odds_api_key:str,odds_api_keys:tuple[str,...]=(),feature_url:str|None=None,projected_lineups_url:str|None=None,provider_token:str|None=None,now:datetime|None=None,opener:Callable=urlopen,registry_path:str="config/deployments.json",require_confirmed_lineup:bool=False,edge_floor_config_path:str=DEFAULT_EDGE_FLOOR_CONFIG,kelly_multiplier:float=0.25,bookmakers:tuple[str,...]=("draftkings",),history_cache_dir:str|Path|None=None)->AutoRunReport:
    ...
    current=current.astimezone(timezone.utc);slate_date_ct=current.astimezone(CHICAGO_TZ).date().isoformat()

    schedule=fetch_schedule(slate_date_ct,opener=opener,now=current);roster_names={};roster_failures=[]
    for game in schedule:
        try:
            box=fetch_boxscore(game.game_pk,opener=opener);roster_names[game.game_pk]=_roster_names(box)
        except Exception as exc:
            roster_names[game.game_pk]=[];roster_failures.append({"stage":"MLB_ROSTER_IDENTITY","game_id":str(game.game_pk),"reason":f"{type(exc).__name__}: {exc}"})
    ...
    acquisition_failures=key_failures+[{"stage":"ODDS_API",**dict(item)} for item in odds["failures"]]+roster_failures
    ...
    if feature_url is not None:
        ...
        report=run_auto_mlb(quote_url=MEMORY_QUOTES_URL,feature_url=feature_url,projected_lineups_url=projected_lineups_url,provider_token=provider_token,now=current,opener=legacy_wrapped,registry_path=registry_path,require_confirmed_lineup=require_confirmed_lineup,edge_floor_config_path=edge_floor_config_path,kelly_multiplier=kelly_multiplier)
    else:
        history_opener=MLBHistoryCachedOpener(target_date=date.fromisoformat(slate_date_ct),cache_dir=history_cache_dir,opener=opener)
        def joint_wrapped(req,timeout=15):
            if _url(req)==MEMORY_QUOTES_URL:return _MemoryResponse(quote_payload)
            return history_opener(req,timeout=timeout)
        report=run_auto_joint_mlb(quote_url=MEMORY_QUOTES_URL,projected_lineups_url=projected_lineups_url,provider_token=provider_token,now=current,opener=joint_wrapped,registry_path=registry_path,require_confirmed_lineup=require_confirmed_lineup,edge_floor_config_path=edge_floor_config_path,kelly_multiplier=kelly_multiplier)
