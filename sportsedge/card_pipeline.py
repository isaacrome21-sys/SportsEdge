...
The pipeline preserves every requested quote: missing games/features/identity are
returned as BLOCKED rows rather than silently dropped. Successful candidates are
assembled with live MLB identity and then passed through the common runtime
orchestrator and deployment registry.
"""
...
def _quote_identity(quote: Mapping[str, Any]) -> tuple[str, str, str, Any, str]:
    ...
    if not game_id or not entity_id or not market or not side:
        raise LiveSlateError("quote identity fields cannot be blank")
...
def run_hitter_card(
    *,
    ...
    seen_quotes: set[tuple[str, str, str, str, str]] = set()
    out: list[CardResult] = []
