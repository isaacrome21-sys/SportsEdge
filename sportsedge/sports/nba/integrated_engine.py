...
SUPPORTED_GAME = frozenset({
    "MONEYLINE", "SPREAD", "TOTAL", "HOME_TEAM_TOTAL", "AWAY_TEAM_TOTAL",
})
SUPPORTED_PERIOD = frozenset({
    "FIRST_HALF_SPREAD", "FIRST_HALF_TOTAL", "QUARTER_SPREAD", "QUARTER_TOTAL",
})
SUPPORTED_PLAYER = frozenset({
    "PLAYER_POINTS", "PLAYER_REBOUNDS", "PLAYER_ASSISTS", "PLAYER_THREES",
    "PLAYER_PRA", "PLAYER_PR", "PLAYER_PA", "PLAYER_RA",
})
...
def build_integrated_paths(
    row: NBATrainingRow,
    pace: NBAPaceEfficiencyModel,
    periods: NBAPeriodParameters,
    roles: Iterable[NBAPlayerStatRole],
    *,
    ...
    state = NBAGameState(
        game_id=row.game_id,
        expected_possessions=predicted["expected_possessions"],
        home_points_per_100=predicted["home_points_per_100"],
        away_points_per_100=predicted["away_points_per_100"],
    )
...
def _player_values(paths: NBAPlayerStatPaths, market: str) -> tuple[int, ...]:
    mapping = {
        "PLAYER_POINTS": paths.points,
        "PLAYER_REBOUNDS": paths.rebounds,
        "PLAYER_ASSISTS": paths.assists,
        "PLAYER_THREES": paths.threes,
        "PLAYER_PRA": paths.pra,
        "PLAYER_PR": paths.pr,
        "PLAYER_PA": paths.pa,
        "PLAYER_RA": paths.ra,
    }
...
def _fair_for_quote(quote: NBAQuote, bundle: NBAIntegratedPaths) -> NBAFairPrice:
    ...
    if cap.status != "REQUIRES_ENGINE":
        raise ValueError(f"unsupported NBA market: {market}")
    selection = quote.selection.strip().upper()
    game = bundle.game
    if market == "MONEYLINE":
        if selection not in {"HOME", "AWAY"}:
            raise ValueError("moneyline selection must be HOME or AWAY")
        return moneyline_probability(game.home_final, game.away_final, home=selection == "HOME")
    if quote.line is None:
        raise ValueError(f"{market} requires a finite line")
    over = selection in {"OVER", "HOME"}
    if market == "SPREAD":
        values = tuple(h - a for h, a in zip(game.home_final, game.away_final))
        return path_outcome(values, quote.line if selection == "HOME" else -quote.line, over=True)
    if market == "TOTAL":
        values = tuple(h + a for h, a in zip(game.home_final, game.away_final))
        return path_outcome(values, quote.line, over=over)
    if market == "HOME_TEAM_TOTAL":
        return path_outcome(game.home_final, quote.line, over=over)
    if market == "AWAY_TEAM_TOTAL":
        return path_outcome(game.away_final, quote.line, over=over)
    if market == "FIRST_HALF_SPREAD":
        return path_outcome(half_margin(bundle.periods, 1), quote.line if selection == "HOME" else -quote.line, over=True)
    if market == "FIRST_HALF_TOTAL":
        return path_outcome(half_total(bundle.periods, 1), quote.line, over=over)
    ...
    if market in SUPPORTED_PLAYER:
        ...
        if player_id not in bundle.players:
            raise ValueError(f"no fitted role for player {player_id}")
        return path_outcome(_player_values(bundle.players[player_id], market), quote.line, over=over)
    raise ValueError(f"unsupported NBA market: {market}")


def bind_integrated_card(
    quotes: Iterable[NBAQuote],
    bundle: NBAIntegratedPaths,
    *,
    as_of: datetime,
    model_quality: float,
    ...
    edges: list[NBABoundEdge] = []
    requested = []
    for quote in quotes:
        requested.append(quote.market)
        cap = capability_for(quote.market)
        if cap.status != "REQUIRES_ENGINE":
            continue
        fair = _fair_for_quote(quote, bundle)
        if calibrator is not None and fair.push_probability == 0:
            wp = calibrator.calibrate(min(0.999, max(0.001, fair.win_probability)))
            lp = 1.0 - wp
            fair = NBAFairPrice(wp, 0.0, lp, 1.0 / wp if wp else float("inf"))
        edges.append(bind_quote(
            quote, fair, as_of=as_of, model_quality=model_quality, context_quality=context_quality,
        ))
