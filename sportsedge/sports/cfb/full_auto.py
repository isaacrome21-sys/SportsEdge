...
from .travel_provider_factory import build_cfbd_enriched_provider_factory
...
def build_cfb_full_auto_slate(
    *,
    ...
    then objective weather, prior-only advanced metrics/matchups, rest, travel,
    timezone shift and destination elevation are acquired where venue identity is
    resolvable. Unsupported trusted classes remain explicit MISSING_PROVIDER;
    ...
    return build_cfb_auto_context_slate(
        ...
        provider_factory=build_cfbd_enriched_provider_factory(cfbd_api_key=cfbd_api_key, opener=opener),
