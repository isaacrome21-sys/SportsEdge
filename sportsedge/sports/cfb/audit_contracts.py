...
They make point-in-time provenance, coverage accounting, narrative overrides and
...
class PITObservation:
    ...
    def validate(self) -> "PITObservation":
        ...
        # Availability must precede the feature cut and kickoff. Ingestion may occur
        # after source publication, but cannot exceed the declared source latency.
...
class CFBAuditManifest:
    ...
    def validate(self) -> "CFBAuditManifest":
        ...
        hashes = (
            self.spec_sha, self.policy_sha, self.benchmark_methodology_sha,
            self.exposure_sha, self.market_context_sha, self.decay_schedule_hash,
        )
        if any(len(str(x)) != 64 for x in hashes):
            raise CFBAuditContractError("MANIFEST_SHA256_REQUIRED")
        return self
