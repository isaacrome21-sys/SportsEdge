...
Candidate selection is separate from promotion evidence. Reconstructed historical
data may support the frozen selection exercise, but it never creates historical
...
CFB_SELECTION_READINESS_VERSION = "CFB_FIRST_EVALUATION_READINESS_V4"
...
def audit_cfb_selection_readiness(
    *,
    ...
    if not acquisition_ready:
        ...
        blockers.extend(str(x) for x in (acquisition_report.get("blockers") or []))
    ...
    if not isinstance(selection_bundle, Mapping):
        bundle_blockers.append("RECONSTRUCTED_SELECTION_BUNDLE_MISSING")
    else:
        if selection_bundle.get("schema") != "CFB_RECONSTRUCTED_SELECTION_BUNDLE_V1":
            bundle_blockers.append("SELECTION_BUNDLE_SCHEMA_MISMATCH")
        if selection_bundle.get("status") != "READY_FOR_CANDIDATE_EVALUATION":
            bundle_blockers.append("SELECTION_BUNDLE_STATUS_NOT_READY")
        expected = {
        ...
        for key, value in expected.items():
            if selection_bundle.get(key) != value:
                bundle_blockers.append(f"SELECTION_BUNDLE_CONTRACT_MISMATCH:{key}")
        if start_season is not None and selection_bundle.get("start_season") != start_season:
            bundle_blockers.append("SELECTION_BUNDLE_START_SEASON_MISMATCH")
        if end_season is not None and selection_bundle.get("end_season") != end_season:
            bundle_blockers.append("SELECTION_BUNDLE_END_SEASON_MISMATCH")
        if selection_bundle.get("no_2026_forward_outcomes") is not True:
            bundle_blockers.append("SELECTION_BUNDLE_FORWARD_OUTCOME_GUARD_MISSING")
        if selection_bundle.get("market_data_in_predictive_features") is not False:
            bundle_blockers.append("SELECTION_BUNDLE_MARKET_CONTAMINATION")
        for key in (
            ...
            if not _valid_sha(selection_bundle.get(key)):
                bundle_blockers.append(f"SELECTION_BUNDLE_HASH_INVALID:{key}")
        ...
        if {"wind_speed", "temperature"} & candidate_features:
            if selection_bundle.get("weather_provenance_class") not in {
                RECONSTRUCTED_PROVENANCE,
                "PRE_EVENT_ARCHIVE",
            }:
                bundle_blockers.append("SELECTION_BUNDLE_WEATHER_PROVENANCE_UNRESOLVED")
            if not _nonempty(selection_bundle.get("weather_source_contract")):
                bundle_blockers.append("SELECTION_BUNDLE_WEATHER_SOURCE_CONTRACT_MISSING")

    blockers.extend(bundle_blockers)
    bundle_ready = not bundle_blockers
    unique_blockers = list(dict.fromkeys(blockers))
    allowed = prereg_ready and acquisition_ready and bundle_ready and not unique_blockers
    return {
