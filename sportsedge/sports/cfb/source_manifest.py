...
_ALLOWED_AVAILABILITY_MODES = {"PRE_EVENT_ARCHIVE", "EVENT_TIMESTAMPED_REPLAY", "POST_EVENT_LABEL"}
...
def validate_cfb_pit_source_manifest(payload: Mapping[str, Any], *, raw_bytes: bytes, fit_max_season: int) -> dict[str, Any]:
    ...
    try:
        manifest_fit_max = int(payload.get("fit_max_season")); expected_fit_max = int(fit_max_season)
    ...
    try:
        min_season = int(window.get("min_season")); max_season = int(window.get("max_season"))
    except (TypeError, ValueError) as exc:
    ...
    source_ids: set[str] = set(); clean_sources: list[dict[str, Any]] = []; roles_seen: set[str] = set()
    for index, item_raw in enumerate(sources):
        ...
        retrieved_at = _aware_timestamp(item.get("retrieved_at_utc"), f"CFB_SOURCE_MANIFEST_RETRIEVED_AT_INVALID:{source_id}")
        content_sha = _hex64(item.get("content_sha256"), f"CFB_SOURCE_MANIFEST_CONTENT_SHA256_INVALID:{source_id}")
        availability_rule = _text(item.get("availability_rule"), f"CFB_SOURCE_MANIFEST_AVAILABILITY_RULE_REQUIRED:{source_id}")
        ...
        clean_sources.append({"source_id": source_id, "role": role, "provider": provider, "dataset": dataset,
            "locator": locator, "snapshot_path": snapshot_path, "retrieved_at_utc": retrieved_at,
            "content_sha256": content_sha, "availability_mode": mode, "availability_rule": availability_rule,
            "market_data": False, "post_cutoff_excluded": True, "seasons": clean_seasons})
    ...
    return {"schema_version": CFB_PIT_SOURCE_MANIFEST_SCHEMA, "manifest_sha256": sha256(raw_bytes).hexdigest(),
        "fit_max_season": expected_fit_max, "training_window": {"min_season": min_season, "max_season": max_season},
        "source_count": len(clean_sources), "source_ids": sorted(source_ids), "sources": clean_sources,
...
def verify_cfb_source_snapshots(manifest: Mapping[str, Any], *, evidence_root: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(sources, list) or not sources:
        raise CFBSourceManifestError("CFB_SOURCE_MANIFEST_SOURCES_REQUIRED")
    verified: list[dict[str, Any]] = []; root_digest = sha256()
    for source in sorted((dict(item) for item in sources), key=lambda item: str(item.get("source_id"))):
        source_id = str(source.get("source_id") or "").strip(); relative = _snapshot_path(source.get("snapshot_path"), source_id)
        ...
        root_digest.update(source_id.encode("utf-8")); root_digest.update(b"\0"); root_digest.update(actual_sha.encode("ascii")); root_digest.update(b"\0")
        verified.append({"source_id": source_id, "snapshot_path": relative, "content_sha256": actual_sha, "byte_count": byte_count})
    return {"source_snapshot_verified": True, "source_content_root_sha256": root_digest.hexdigest(),
        "verified_source_count": len(verified), "verified_sources": verified}


__all__ = ["CFB_PIT_SOURCE_MANIFEST_SCHEMA", "CFBSourceManifestError", "validate_cfb_pit_source_manifest", "verify_cfb_source_snapshots"]
