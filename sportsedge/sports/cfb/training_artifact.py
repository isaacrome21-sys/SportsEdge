...
match every content hash declared by the manifest. By default, ridge alpha is
selected only from season-ordered prior-season folds. An explicitly supplied
fixed alpha remains available for controlled compatibility/diagnostic use and
is labeled separately in provenance.
"""
...
def _surface_sha256(repo_root: str | Path, paths: tuple[str, ...], *, error_prefix: str) -> str:
    ...
    for relative in paths:
        ...
        try:
            ...
            raise CFBTrainingArtifactError(f"{error_prefix}:{relative}") from exc
...
def cfb_training_code_surface_sha256(repo_root: str | Path) -> str:
    return _surface_sha256(
        repo_root,
        CFB_TRAINING_CODE_SURFACE,
        error_prefix="CFB_TRAINING_CODE_SURFACE_UNREADABLE",
    )
...
def cfb_training_derivation_code_sha256(repo_root: str | Path) -> str:
    return _surface_sha256(
        repo_root,
        CFB_TRAINING_DERIVATION_SURFACE,
        error_prefix="CFB_TRAINING_DERIVATION_SURFACE_UNREADABLE",
    )
...
def validate_cfb_pit_training_bundle(payload: Mapping[str, Any], *, raw_bytes: bytes, fit_max_season: int) -> dict[str, Any]:
    ...
    return {
        ...
        "game_ids_sha256": _game_ids_sha256(list(game_ids)),
        "train_seasons": sorted(seasons),
        "fit_max_season": fit_max,
        "generated_at_utc": generated_at,
        "source_manifest_sha256": source_manifest_sha,
        "training_bundle_sha256": sha256(raw_bytes).hexdigest(),
...
def build_cfb_artifact_from_pit_bundle(
    ...
    try:
        manifest = validate_cfb_pit_source_manifest(
            source_manifest,
    ...
    provenance = {
        ...
        "upstream_source_manifest_sha256": manifest["manifest_sha256"],
        ...
        "training_seed_policy": CFB_TRAINING_SEED_POLICY,
...
__all__ = [
    ...
    "CFB_TRAINING_CODE_SURFACE",
    "CFB_TRAINING_DERIVATION_SURFACE",
    "CFB_TRAINING_SEED_POLICY",
    "CFBTrainingArtifactError",
    "build_cfb_artifact_from_pit_bundle",
    "cfb_training_code_surface_sha256",
    "cfb_training_derivation_code_sha256",
