...
Raw upstream bytes are preserved for provenance.  The upstream play-by-play file
contains sportsbook-derived fields, so it is never itself a predictive model
input.  A deterministic allowlist projection strips the raw PBP to market-blind
football state before any participation-model materializer may consume it.

Snapshots establish availability only from retrieval time forward. They do not
backfill point-in-time history, create Model_P, or grant promotion authority.
"""
...
PARTICIPATION_CAPTURE_CONTRACT = "CFB_PARTICIPATION_SOURCE_CAPTURE_V2"
PBP_PROJECTION_CONTRACT = "CFB_PBP_MARKET_BLIND_PROJECTION_V1"
...
USER_AGENT = "SportsEdge-CFB-Participation/2"

# Exact source allowlist for the PBP projection.  The upstream PBP release also
# contains gameSpread, overUnder, homeTeamSpread, start.pos_team_spread,
# spread_time and other spread-influenced fields.  Nothing outside this mapping
# can cross the source/model firewall.
PBP_SOURCE_TO_CANONICAL = {
    ...
    "start.homeScore": "home_score_start",
    "start.awayScore": "away_score_start",
    "start.yardsToEndzone": "yards_to_endzone_start",
    "type.text": "play_type",
    "orig_play_type": "orig_play_type",
    "status_type_completed": "status_completed",
}
PBP_PREDICTIVE_COLUMNS = tuple(PBP_SOURCE_TO_CANONICAL.values())
...
def select_participation_asset(
    ...
    usage = (
        "RAW_MIXED_SOURCE_PROVENANCE_ONLY"
        if name == "play_by_play"
        else "PREDICTIVE_PARTICIPATION_INPUT"
    )
...
def sanitize_pbp_projection(
    *, asset: CFBReleaseAsset, source_path: str | Path, projection_path: str | Path
) -> dict[str, Any]:
    """Create the only PBP representation allowed to cross into model materialization."""
    if asset.dataset != "play_by_play":
        raise CFBParticipationSourceError("CFB_PBP_PROJECTION_DATASET_INVALID")
    ...
    try:
        with partial.open("w", encoding="utf-8", newline="") as handle:
            writer = csv.DictWriter(
                handle,
                fieldnames=list(PBP_PREDICTIVE_COLUMNS),
                extrasaction="raise",
                lineterminator="\n",
            )
            writer.writeheader()
            for raw in iter_cached_csv(asset, str(source_path)):
                missing = [field for field in PBP_SOURCE_TO_CANONICAL if field not in raw]
                if missing:
                    raise CFBParticipationSourceError(
                        "CFB_PBP_PROJECTION_SOURCE_FIELD_MISSING:" + ",".join(missing)
                    )
                writer.writerow({
                    canonical: raw[source]
                    for source, canonical in PBP_SOURCE_TO_CANONICAL.items()
                })
                row_count += 1
        if row_count <= 0:
            raise CFBParticipationSourceError("CFB_PBP_PROJECTION_EMPTY")
    ...
    return {
        ...
        "projection_relative_path": None,
        "projection_sha256": _file_sha256(target),
        "projection_row_count": row_count,
        "source_content_sha256": asset.sha256,
        "source_fields": list(PBP_SOURCE_TO_CANONICAL),
        "predictive_columns": list(PBP_PREDICTIVE_COLUMNS),
        "sanitizer_code_sha256": _file_sha256(Path(__file__)),
...
def cache_participation_source(
    *,
    ...
    projection = None
    if name == "play_by_play":
        projection_path = target.parent / "predictive" / "play_by_play_market_blind.csv"
        projection = sanitize_pbp_projection(
            asset=asset, source_path=target, projection_path=projection_path
        )
        projection["projection_relative_path"] = str(
            Path(name) / str(year) / "predictive" / projection_path.name
        )
        row_count = int(projection["projection_row_count"])
    else:
        row_count = sum(1 for _ in iter_cached_csv(asset, str(target)))
    ...
    manifest.update(
        {
            ...
            "raw_predictive_input_allowed": not raw_market,
