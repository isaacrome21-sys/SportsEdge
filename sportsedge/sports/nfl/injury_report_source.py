...
def build_pit_injury_inputs(
    *,
    ...
    Multiple updates for the same player can exist. We retain only the latest
    `date_modified <= as_of` row for each `(team, gsis_id)` and never infer a
    status for a player who has no eligible row.
    """
    ...
    for raw in rows:
        ...
        if modified_raw in (None, ""):
            # Freshness is unverifiable; fail closed at row scope.
            continue
