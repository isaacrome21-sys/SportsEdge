"""Walk-forward diagnostic evidence for the NFL M2 V2E crossfit-support candidate."""
...
_KEY_PROFILE_CONTRACT = "NFL_M2_V2E_CANDIDATE_OOS_SIGNED_KEY_PMF_V1"
...
def build_nfl_m2_v2e_raw_evaluations(
    ...
    *,
    ...
    for fold in folds:
        model = fit_nfl_m2_v2e_candidate(
            fold.train_rows,
            ridge_alpha=ridge_alpha,
            margin_kernel_scale=margin_kernel_scale,
            total_kernel_scale=total_kernel_scale,
            min_coordinate_train_seasons=min_coordinate_train_seasons,
        )
        if int(fold.test_season) in model.train_seasons:
            raise ValueError("NFL_M2_V2E_TEST_SEASON_IN_TRAINING")
        if any(point.support_season >= int(fold.test_season) for point in model.support_points):
            raise ValueError("NFL_M2_V2E_SUPPORT_NOT_STRICTLY_BEFORE_OUTER_TEST")
        if any(
            max(point.coordinate_train_seasons) >= point.support_season
            for point in model.support_points
            if point.coordinate_train_seasons
        ):
            raise ValueError("NFL_M2_V2E_SUPPORT_COORDINATE_TEMPORAL_LEAKAGE")

        for raw in fold.test_rows:
            row = dict(raw)
            distribution = derive_nfl_m2_v2e_score_distribution(model, row)
            key_probabilities = {
                str(key): sum(
                    float(score["weight"])
            ...
            output.append({
                ...
                "dropped_support_seasons": model.dropped_support_seasons,
                "min_coordinate_train_seasons": model.min_coordinate_train_seasons,
...
def _key_profile(evaluations: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "probability_source": "OOS_STRUCTURED_MEAN_SEASON_CROSSFIT_EMPIRICAL_INTEGER_SCORE_SUPPORT",
        "heldout_game_count": len(rows),
        "test_seasons": sorted({int(row["season"]) for row in rows}),
        "signed_key_probability": {
            str(key): totals[key] / denominator for key in _KEY_NUMBERS
        },
    }
...
def build_nfl_m2_v2e_candidate_evidence(
    ...
    *,
    ...
    for season in sorted({int(row["season"]) for row in raw}):
        season_rows = [row for row in raw if int(row["season"]) == season]
        if not season_rows:
            continue
        exemplar = season_rows[0]
        support_audit.append({
            "outer_test_season": season,
            "outer_train_seasons": list(exemplar["train_seasons"]),
            "dropped_support_seasons": list(exemplar["dropped_support_seasons"]),
            "support_point_count": int(exemplar["support_point_count"]),
            "min_coordinate_train_seasons": int(exemplar["min_coordinate_train_seasons"]),
        })
    ...
    return {
        ...
        "crossfit_support_audit": support_audit,
        "parameters": {
            ...
            "margin_kernel_scale": float(margin_kernel_scale),
            "total_kernel_scale": float(total_kernel_scale),
            "min_coordinate_train_seasons": int(min_coordinate_train_seasons),
        },
        "diagnostic_question": (
            "Does replacing self-fit empirical-support coordinates with season-ordered prior-only "
            "coordinates improve untouched-fold spread/total evidence, calibration, and emergent "
            "signed 3/7 mass without changing V2D mean structure, bandwidths, or frozen gates?"
        ),
    }
