...
SCORING_POINTS = {"TD": 6, "FG": 3, "TURNOVER": 0, "PUNT_OTHER": 0, "SAFETY": 2, "DEF_ST_SCORE": 0}
...
def fit_hierarchical_strength(rows: Sequence[DriveRow]) -> HierarchicalStrength:
    ...
    raw_team_td = []
    for group in by_off.values():
        raw_team_td.append(sum(r.outcome == "TD" for r in group) / len(group))
    ...
    for team, group in by_off.items():
        ...
        shrinkage[team] = w
        off_effect[team] = {
            o: w * ((sum(r.outcome == o for r in group) / len(group)) - league[o]) for o in DRIVE_OUTCOMES
        }
...
def _draw(cdf_probs: Mapping[str, float], rng: Random) -> str:
    u = rng.random()
    c = 0.0
    last = DRIVE_OUTCOMES[-1]
    for outcome in DRIVE_OUTCOMES:
        c += float(cdf_probs[outcome])
        if u <= c:
            return outcome
        last = outcome
    return last
...
def simulate_joint_game(
    ...
    *,
    ...
    total_drives = max_regulation_drives
    ot_drives = 0

    while state.drive_index < total_drives or (state.home_score == state.away_score and ot_drives < max_overtime_drives):
        ...
        probs = model.probabilities(offense, defense)
        outcome = _draw(probs, rng)
        conv = 0
        pts_off, pts_def = 0, 0
        if outcome == "TD":
        ...
        state = GameState(home_team, away_team, next_possession, home, away, 5 if in_ot else min(4, 1 + state.drive_index // max(1, max_regulation_drives // 4)), 0 if in_ot else max(0, 3600 - int(3600 * (state.drive_index + 1) / max_regulation_drives)), state.drive_index + 1, in_ot)

        if in_ot and state.home_score != state.away_score and ot_drives >= 2:
            break
    ...
    return SimulationResult(
        ...
        state.home_score + state.away_score,
        {home_team: state.home_score, away_team: state.away_score},
        tuple(path),
    )
