...
def _integer_score(value: float) -> int:
    # Deterministic half-up rounding for non-negative football scores.
...
def fit_nfl_m2_v2d_candidate(
    ...
    *,
    ...
    if not train_seasons:
        raise ValueError("NFL_M2_V2D_TRAINING_SEASONS_REQUIRED")
    residual_pairs = tuple(
        (float(home), float(away))
        for home, away in zip(home_residual.tolist(), away_residual.tolist())
    )
...
def derive_nfl_m2_v2d_score_distribution(
    ...
    return tuple(
        {
            "home_score": _integer_score(home_mu + home_residual),
            "away_score": _integer_score(away_mu + away_residual),
        }
        for home_residual, away_residual in model.residual_pairs
    )
