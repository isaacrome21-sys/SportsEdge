"""Walk-forward diagnostic evidence for the NFL M2 V2C candidate."""
...
_KEY_PROFILE_CONTRACT = "NFL_M2_V2C_CANDIDATE_OOS_SIGNED_KEY_PMF_V1"
...
def build_nfl_m2_v2c_raw_evaluations(
    ...
    *,
    ...
    for fold in folds:
        model = fit_nfl_m2_v2c_candidate(fold.train_rows, ridge_alpha=ridge_alpha)
        ...
        for raw in fold.test_rows:
            ...
            output.append({
                ...
                "residual_support_size": len(model.residuals),
    ...
    for row in rows:
        ...
        for key in _KEY_NUMBERS:
            ...
            if not 0.0 <= value <= 1.0:
                raise ValueError("NFL_M2_V2C_KEY_PROFILE_VALUE_INVALID")
            totals[key] += value
    n = float(len(rows))
    return {
        ...
        "probability_source": "OOS_TRAINING_RESIDUAL_TRANSLATION",
...
def build_nfl_m2_v2c_candidate_evidence(
    ...
    *,
    ...
    raw = build_nfl_m2_v2c_raw_evaluations(
        rows,
    ...
    return {
        ...
        "diagnostic_question": (
            "Can training-only score residual translation preserve the discrete NFL scoring lattice "
            "while improving held-out spread/total probabilities without market-derived fitting?"
        ),
    }
