...
def _clv_row(clv_bundle: Mapping[str,Any]|None, market: str) -> Mapping[str,Any]|None:
    if not clv_bundle:
        return None
    if clv_bundle.get("schema_version")!=CLV_BUNDLE_SCHEMA:
        raise ValueError("NFL_V2G_EVAL_CLV_BUNDLE_SCHEMA_INVALID")
    ...
    if row is not None and row.get("schema_version")!=CLV_ROW_SCHEMA:
        raise ValueError("NFL_V2G_EVAL_CLV_ROW_SCHEMA_INVALID")
    return row


def _validate_eligible_clv_row(clv: Mapping[str,Any], decision: Mapping[str,Any]) -> float:
    if clv.get("status")!="CLV_ELIGIBLE":
        raise ValueError("NFL_V2G_EVAL_CLV_STATUS_INVALID")
    if str(clv.get("game_id") or "")!=str(decision.get("game_id") or ""):
        raise ValueError("NFL_V2G_EVAL_CLV_GAME_MISMATCH")
    if str(clv.get("market") or "")!=str(decision.get("market") or ""):
        raise ValueError("NFL_V2G_EVAL_CLV_MARKET_MISMATCH")
    if str(clv.get("side") or "")!=str(decision.get("side") or ""):
        raise ValueError("NFL_V2G_EVAL_CLV_SIDE_MISMATCH")
    ...
    if abs(clv_decision_novig-decision_novig)>1e-12:
        raise ValueError("NFL_V2G_EVAL_CLV_DECISION_NOVIG_MISMATCH")
    observed=_num(clv.get("clv"),"NFL_V2G_EVAL_CLV_INVALID")
    expected=closing_novig-decision_novig
    if abs(observed-expected)>1e-12:
        raise ValueError("NFL_V2G_EVAL_CLV_METRIC_MISMATCH")
    return observed
...
def build_settlement(decision_bundle: Mapping[str,Any], binding: Mapping[str,Any]|None, clv_bundle: Mapping[str,Any]|None,
    ...
    for d in rows:
        ...
        settled.append({
            ...
            "clv":clv_value,"clv_metric":CLV_METRIC if clv_value is not None else None,
...
def evaluate_checkpoint(settlements: Iterable[Mapping[str,Any]], *, count: int, policy: Mapping[str,Any]) -> dict[str,Any]:
    ...
    for bundle in settlements:
        for row in bundle.get("rows") or []:
            if row.get("eligible_for_checkpoint"):
                if row.get("clv_metric")!=CLV_METRIC:
                    raise ValueError("NFL_V2G_EVAL_CHECKPOINT_CLV_METRIC_INVALID")
    ...
    return {
        ...
        "metrics":{"mean_clv":mean_clv,"after_vig_roi":roi,"calibration_n":len(calibration),"calibration_slope":slope,"calibration_intercept":intercept,"ece":ece,"max_nonempty_bin_abs_deviation":maxdev,"calibration_error":calibration_error,"clv_metric":CLV_METRIC},
