...
def _dt(value: Any) -> datetime:
    raw=str(value or "").strip(); out=datetime.fromisoformat(raw[:-1]+"+00:00" if raw.endswith("Z") else raw)
...
def validate_policy(policy: Mapping[str,Any]) -> None:
    if policy.get("schema_version")!=POLICY_SCHEMA or policy.get("status")!="FROZEN_BEFORE_FIRST_WEEK2_OUTCOME": raise ValueError("NFL_V2G_EVAL_POLICY_INVALID")
    source=policy.get("outcome_source") or {}
    if source.get("contract")!=OUTCOME_SCHEMA or source.get("direct_schedule_refetch_for_evaluation") is not False: raise ValueError("NFL_V2G_EVAL_OUTCOME_SOURCE_POLICY_INVALID")
    if float(source.get("minimum_hours_after_kickoff",0.0))<8.0: raise ValueError("NFL_V2G_EVAL_OUTCOME_DELAY_POLICY_INVALID")
...
def _decision_identity(decision_bundle: Mapping[str,Any]) -> tuple[str,str,str,str]:
    game_id=str(decision_bundle.get("game_id") or ""); rows=list(decision_bundle.get("rows") or [])
    if not rows: raise ValueError("NFL_V2G_EVAL_DECISION_ROWS_EMPTY")
    prediction_shas={_hex(r.get("prediction_sha256"),64,"NFL_V2G_EVAL_DECISION_PREDICTION_SHA_INVALID") for r in rows}
    artifact_shas={_hex(r.get("artifact_sha256"),64,"NFL_V2G_EVAL_DECISION_ARTIFACT_SHA_INVALID") for r in rows}
    model_ids={str(r.get("model_id") or "") for r in rows}
    if len(prediction_shas)!=1 or len(artifact_shas)!=1 or len(model_ids)!=1 or not next(iter(model_ids)): raise ValueError("NFL_V2G_EVAL_DECISION_IDENTITY_INCONSISTENT")
    return game_id,next(iter(prediction_shas)),next(iter(artifact_shas)),next(iter(model_ids))


def _validate_binding(binding: Mapping[str,Any], decision_bundle: Mapping[str,Any]) -> None:
    if binding.get("schema_version")!=BINDING_SCHEMA: raise ValueError("NFL_V2G_EVAL_BINDING_SCHEMA_INVALID")
    game_id,prediction_sha,artifact_sha,model_id=_decision_identity(decision_bundle)
    if str(binding.get("game_id") or "")!=game_id: raise ValueError("NFL_V2G_EVAL_BINDING_GAME_MISMATCH")
    if str(binding.get("candidate_id") or "")!=model_id: raise ValueError("NFL_V2G_EVAL_BINDING_MODEL_MISMATCH")
    pred=binding.get("prediction") or {}
    if str(pred.get("prediction_sha256") or "").lower()!=prediction_sha: raise ValueError("NFL_V2G_EVAL_BINDING_PREDICTION_MISMATCH")
    if str(pred.get("artifact_sha256") or "").lower()!=artifact_sha: raise ValueError("NFL_V2G_EVAL_BINDING_ARTIFACT_MISMATCH")
    if binding.get("market_prices_consumed_by_model") is not False: raise ValueError("NFL_V2G_EVAL_BINDING_MARKET_LEAKAGE")
    for key in ("promotion_authority","may_create_model_p","market_eligibility_changed","truth_gate_pass_granted","official_status_granted"):
        if binding.get(key) is not False: raise ValueError(f"NFL_V2G_EVAL_BINDING_AUTHORITY_INVALID:{key}")
...
def _clv_row(clv_bundle: Mapping[str,Any]|None, market: str) -> Mapping[str,Any]|None:
    ...
    rows=clv_bundle.get("rows") or []; matches=[r for r in rows if r.get("market")==market]
...
def _validate_eligible_clv_row(clv: Mapping[str,Any], decision: Mapping[str,Any]) -> float:
    ...
    if abs(_prob(clv.get("decision_model_prob"),"NFL_V2G_EVAL_CLV_MODEL_PROB_INVALID")-_prob(decision.get("model_prob"),"NFL_V2G_EVAL_MODEL_PROB_INVALID"))>1e-12: raise ValueError("NFL_V2G_EVAL_CLV_MODEL_PROB_MISMATCH")
    decision_novig=_prob(decision.get("novig_prob"),"NFL_V2G_EVAL_DECISION_NOVIG_INVALID"); clv_decision_novig=_prob(clv.get("decision_novig_prob"),"NFL_V2G_EVAL_CLV_DECISION_NOVIG_INVALID"); closing_novig=_prob(clv.get("closing_novig_prob"),"NFL_V2G_EVAL_CLV_CLOSING_NOVIG_INVALID")
    if abs(clv_decision_novig-decision_novig)>1e-12: raise ValueError("NFL_V2G_EVAL_CLV_DECISION_NOVIG_MISMATCH")
    observed=_num(clv.get("clv"),"NFL_V2G_EVAL_CLV_INVALID"); expected=closing_novig-decision_novig
...
def _outcome_sha256(outcome: Mapping[str,Any]) -> str:
    payload=dict(outcome); payload.pop("outcome_sha256",None); return sha256(canonical_bytes(payload)).hexdigest()


def validate_canonical_outcome(outcome: Mapping[str,Any], decision_bundle: Mapping[str,Any], policy: Mapping[str,Any]) -> None:
    validate_policy(policy)
    if outcome.get("schema_version")!=OUTCOME_SCHEMA or outcome.get("status")!="PROSPECTIVE_RESEARCH_OUTCOME_CAPTURED": raise ValueError("NFL_V2G_EVAL_OUTCOME_SCHEMA_INVALID")
    ...
    game_id,prediction_sha,artifact_sha,_=_decision_identity(decision_bundle)
    ...
    if _hex(outcome.get("prediction_sha256"),64,"NFL_V2G_EVAL_OUTCOME_PREDICTION_SHA_INVALID")!=prediction_sha: raise ValueError("NFL_V2G_EVAL_OUTCOME_PREDICTION_MISMATCH")
    if _hex(outcome.get("artifact_sha256"),64,"NFL_V2G_EVAL_OUTCOME_ARTIFACT_SHA_INVALID")!=artifact_sha: raise ValueError("NFL_V2G_EVAL_OUTCOME_ARTIFACT_MISMATCH")
    _hex(outcome.get("result_schedule_snapshot_sha256"),64,"NFL_V2G_EVAL_OUTCOME_SNAPSHOT_SHA_INVALID"); _hex(outcome.get("prediction_capture_code_git_sha"),40,"NFL_V2G_EVAL_OUTCOME_CAPTURE_CODE_SHA_INVALID"); _hex(outcome.get("settlement_code_git_sha"),40,"NFL_V2G_EVAL_OUTCOME_SETTLEMENT_CODE_SHA_INVALID")
    if str(outcome.get("outcome_sha256") or "").lower()!=_outcome_sha256(outcome): raise ValueError("NFL_V2G_EVAL_OUTCOME_SHA_MISMATCH")
    rows=list(decision_bundle.get("rows") or []); kickoff=_dt(outcome.get("kickoff_utc")); observed=_dt(outcome.get("observed_at_utc"))
    if kickoff!=_dt(rows[0].get("game_start_ts")): raise ValueError("NFL_V2G_EVAL_OUTCOME_KICKOFF_MISMATCH")
    recorded_delay=_num(outcome.get("minimum_hours_after_kickoff"),"NFL_V2G_EVAL_OUTCOME_DELAY_INVALID"); required_delay=float(policy["outcome_source"]["minimum_hours_after_kickoff"])
    ...
    home_score=_num(outcome.get("home_score"),"NFL_V2G_EVAL_HOME_SCORE_MISSING"); away_score=_num(outcome.get("away_score"),"NFL_V2G_EVAL_AWAY_SCORE_MISSING")
...
def build_settlement(decision_bundle: Mapping[str,Any], binding: Mapping[str,Any]|None, clv_bundle: Mapping[str,Any]|None, outcome: Mapping[str,Any], *, policy: Mapping[str,Any]) -> dict[str,Any]:
    validate_canonical_outcome(outcome,decision_bundle,policy)
    if binding is not None: _validate_binding(binding,decision_bundle)
    ...
    away,home=_game_codes(game_id); rows=list(decision_bundle.get("rows") or []); kickoff=_dt(rows[0].get("game_start_ts")); home_score=_num(outcome.get("home_score"),"NFL_V2G_EVAL_HOME_SCORE_MISSING"); away_score=_num(outcome.get("away_score"),"NFL_V2G_EVAL_AWAY_SCORE_MISSING"); margin,total=home_score-away_score,home_score+away_score; settled=[]; min_edge=float(policy["eligible_bet"]["minimum_decision_edge"])
    for d in rows:
        ...
        result="WIN" if value>0 else "LOSS" if value<0 else "PUSH"; profit=_profit(d.get("price_at_decision")) if result=="WIN" else -1.0 if result=="LOSS" else 0.0; clv=_clv_row(clv_bundle,market); reasons=[]; clv_value=None
...
def evaluate_checkpoint(settlements: Iterable[Mapping[str,Any]], *, count: int, policy: Mapping[str,Any]) -> dict[str,Any]:
    ...
    rows=eligible[:count]; mean_clv=sum(_num(r.get("clv"),"NFL_V2G_EVAL_CLV_INVALID") for r in rows)/count; roi=sum(_num(r.get("profit_units"),"NFL_V2G_EVAL_PROFIT_INVALID") for r in rows)/count; calibration=[(_num(r.get("model_prob"),"NFL_V2G_EVAL_MODEL_PROB_INVALID"),1 if r.get("result")=="WIN" else 0) for r in rows if r.get("result")!="PUSH"]; bins=int(policy["gates"]["calibration_bins"]); ece,maxdev=_ece(calibration,bins)
