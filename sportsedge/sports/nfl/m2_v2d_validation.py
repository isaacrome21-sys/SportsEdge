...
def build_nfl_m2_v2d_raw_evaluations(
    ...
    for fold in folds:
        ...
        for raw in fold.test_rows:
            ...
            if spread_line is not None:
                wins = sum(1 for margin in margins if margin > spread_line)
                losses = sum(1 for margin in margins if margin < spread_line)
                candidate_home = _conditional_probability(wins, losses)
            if total_line is not None:
                wins = sum(1 for total in totals if total > total_line)
                losses = sum(1 for total in totals if total < total_line)
                candidate_over = _conditional_probability(wins, losses)
