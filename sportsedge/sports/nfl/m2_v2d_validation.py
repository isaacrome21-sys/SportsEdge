"""Historical walk-forward diagnostics for NFL M2 V2D direct-score candidate."""
...
_KEY_PROFILE_CONTRACT = "NFL_M2_V2D_OOS_SIGNED_KEY_PMF_V1"
...
def build_nfl_m2_v2d_raw_evaluations(rows: Iterable[dict[str, Any]], *, min_train_seasons: int = 2, ridge_alpha: float = 10.0) -> list[dict[str, Any]]:
    ...
    for fold in folds:
        model = fit_nfl_m2_v2d_candidate(fold.train_rows, ridge_alpha=ridge_alpha)
        if int(fold.test_season) in model.train_seasons:
            raise ValueError("NFL_M2_V2D_TEST_SEASON_IN_TRAINING")
        for raw in fold.test_rows:
            row = dict(raw)
            distribution = derive_nfl_m2_v2d_score_distribution(model, row)
            ...
            key_probabilities = {str(key): sum(1 for margin in margins if margin == key) / len(margins) for key in _KEY_NUMBERS}
            spread_line = _float(row.get("spread_line"))
            total_line = _float(row.get("total_line"))
            candidate_home = None
            candidate_over = None
            if spread_line is not None:
                candidate_home = _conditional_probability(sum(1 for margin in margins if margin > spread_line), sum(1 for margin in margins if margin < spread_line))
            if total_line is not None:
                candidate_over = _conditional_probability(sum(1 for total in totals if total > total_line), sum(1 for total in totals if total < total_line))
            home_score = _float(row.get("home_score"))
            away_score = _float(row.get("away_score"))
            if home_score is None or away_score is None:
                raise ValueError("NFL_M2_V2D_REALIZED_SCORE_MISSING")
...
def _loss(pairs: list[tuple[int, float]]) -> tuple[float, float]:
    if not pairs:
        raise ValueError("NFL_M2_V2D_EMPTY_EVALUATION")
    ll = brier = 0.0
    ...
    for season in sorted({int(row["season"]) for row in data}):
        ...
        for market, (outcome_key, baseline_key, candidate_key) in specs.items():
            ...
            output.append({
                ...
                "candidate_probability_source": "DIRECT_HOME_AWAY_SCORE_PAIRED_RESIDUAL_PLUS_FOLD_SAFE_ISOTONIC",
            })
    return output
...
def _key_profile(evaluations: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not rows:
        raise ValueError("NFL_M2_V2D_KEY_PROFILE_EMPTY")
    totals = {key: 0.0 for key in _KEY_NUMBERS}
    for row in rows:
        payload = row.get("candidate_signed_key_probability")
        if not isinstance(payload, dict):
            raise ValueError("NFL_M2_V2D_KEY_PROFILE_ROW_MISSING")
        for key in _KEY_NUMBERS:
            value = float(payload[str(key)])
            if not 0.0 <= value <= 1.0:
                raise ValueError("NFL_M2_V2D_KEY_PROFILE_VALUE_INVALID")
            totals[key] += value
    n = float(len(rows))
    return {
        ...
        "probability_source": "OOS_DIRECT_SCORE_PAIRED_TRAINING_RESIDUALS",
...
def build_nfl_m2_v2d_candidate_evidence(rows: Iterable[dict[str, Any]], *, source_manifest_sha256: str, min_train_seasons: int = 2, min_calibration_fit_seasons: int = 2, calibration_bins: int = 10, calibration_min_bin_n: int = 25, calibration_threshold: float = 0.05, fold_win_threshold: float = 0.65, ridge_alpha: float = 10.0) -> dict[str, Any]:
    manifest = str(source_manifest_sha256 or "").strip().lower()
    if len(manifest) != 64:
        raise ValueError("NFL_M2_V2D_SOURCE_MANIFEST_SHA256_INVALID")
    try:
        int(manifest, 16)
    except ValueError as exc:
        raise ValueError("NFL_M2_V2D_SOURCE_MANIFEST_SHA256_INVALID") from exc
    raw = build_nfl_m2_v2d_raw_evaluations(rows, min_train_seasons=min_train_seasons, ridge_alpha=ridge_alpha)
    ...
    return {
        ...
        "diagnostic_question": "Does direct home/away score targeting improve held-out spread/total evidence and emergent signed 3/7 mass without using market inputs or changing frozen gates?",
    }
