...
def _empirical_binary_probability(win_probability: float, loss_probability: float, n: int) -> float | None:
    ...
    if n <= 0:
        return None
    win = int(round(float(win_probability) * n))
    loss = int(round(float(loss_probability) * n))
    if win + loss <= 0:
        return None
    ...
    *,
    ...
    for fold in folds:
        ...
        for raw in fold.test_rows:
            row = dict(raw)
            distribution = derive_nfl_m2_score_distribution(model, row)
            n = len(distribution)
            ...
            if spread_line is not None and total_line is not None:
                pricing = price_nfl_m2_game_markets(
                    distribution, spread_line=spread_line, total_line=total_line,
                )
...
def _log_loss(pairs: list[tuple[int, float]]) -> float:
    ...
    return sum(
        -(outcome * log(_clip(probability)) + (1 - outcome) * log(1.0 - _clip(probability)))
...
def _brier(pairs: list[tuple[int, float]]) -> float:
    ...
    return sum((_clip(probability) - outcome) ** 2 for outcome, probability in pairs) / len(pairs)
...
def build_production_nfl_validation_evidence(
    ...
    *,
    ...
    raw = build_production_nfl_raw_evaluations(
        rows, min_train_seasons=min_train_seasons, ridge_alpha=ridge_alpha,
    )
    ...
    for market, evidence in promotion.items():
        ...
        record["production_logic_pass"] = bool(
            evidence.fold_total > 0 and evidence.fold_win_rate >= float(fold_win_threshold)
        )
        record["required_fold_win_rate"] = float(fold_win_threshold)
    ...
    return {
        ...
        "evidence_note": (
            "Exact production model identity. M2 is fit only on prior-season market-blind features; "
            "paired training residuals generate the joint score distribution; closing lines enter only "
            "after that distribution exists; isotonic calibration is fit only on prior OOS seasons."
        ),
    }
