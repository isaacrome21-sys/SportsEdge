...
# Team home time zones. "LA" is the nflverse code for the Rams.
TEAM_TZ = {
...
def classify_venue(game: dict) -> str:
    ...
    return NEUTRAL_DOMESTIC if country.upper() == "US" else NEUTRAL_INTERNATIONAL
...
def evaluate_venue_gate(game: dict, m2_hfa_prior: float, policy: dict) -> dict:
    ...
    if m2_hfa_prior is None or m2_hfa_prior < 0:
    ...
    out["ledger_segment"] = policy["ledger_segments"][venue_class]
    out["in_primary_confirmation_test"] = out["ledger_segment"] in policy["primary_confirmation_test_segments"]
    ...
    try:
        out["travel"] = travel_differential(
            ...
            int(policy["travel"]["tz_shift_wrap_hours"]),
        )
    except VenueContractError as exc:
        out["blockers"].append(exc.code)
