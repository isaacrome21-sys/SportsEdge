...
def _ou(mid,family,entity,period=FULL_GAME,*,threshold_domain,**kw):
    return MarketBinding(mid,family,TWO_WAY,period,entity,("OVER","UNDER"),INTEGER_PUSHABLE,threshold_domain,MULTIPLICATIVE_2WAY,SETTLE_STAT_LINE,True,**kw)
...
for m in ("HITS","TOTAL_BASES","HOME_RUNS","RBI","RUNS","SINGLES","DOUBLES","TRIPLES","EXTRA_BASE_HITS","BATTER_BB","BATTER_K","STOLEN_BASES","HITS_RUNS_RBIS","RUNS_RBIS","HITS_STOLEN_BASES","HITS_RUNS_STOLEN_BASES","HITS_WALKS_STOLEN_BASES"):
    _add(_ou(m,"BATTER_PROP",BATTER,threshold_domain=THRESHOLD_DOMAIN_NON_NEGATIVE,void_on_lineup_absence=True,requires_confirmed_lineup=True))
for m in ("PITCHER_K","PITCHER_OUTS","PITCHER_HITS_ALLOWED","PITCHER_BB","PITCHER_ER","PITCHER_HITS_WALKS_ER"):
    _add(_ou(m,"PITCHER_PROP",PITCHER,threshold_domain=THRESHOLD_DOMAIN_NON_NEGATIVE,void_on_scratch=True))
