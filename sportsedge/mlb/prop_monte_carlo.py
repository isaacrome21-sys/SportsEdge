...
SportsEdge defaults to 50,000 draws so simulation noise is materially smaller
than the thin edges this layer may later help price. Simulation output remains
research pricing evidence only and cannot promote V6.
"""
...
def simulate_count_line(
    *,
    ...
    if simulations < MIN_SIMULATIONS:
        raise ValueError(f"at least {MIN_SIMULATIONS} simulations required")
    rng = Random(seed)
    over = under = push = 0
