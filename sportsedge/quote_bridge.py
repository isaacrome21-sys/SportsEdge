...
def validate_canonical_quote(raw: Mapping[str, Any], *, default_ttl_seconds: int = 300) -> dict[str, Any]:
    ...
    line = 0.0 if raw_line is None and market in LINE_OPTIONAL_MARKETS else _finite("line", raw_line)
    ...
    # Preserve acquisition-origin binding identity exactly. Missing fields remain
    # missing so the orchestration binding gate can block them; normalization does
    # not infer or back-fill identity from the model, schedule, or paired offer.
    for key in (
        "sportsbook", "offer_id", "source_url", "selection", "event_id",
        "provider_event_id", "team_id", "player_id", "event_home_team_id",
        "event_away_team_id", "outcome_id", "outcome_player_id",
    ):
        if raw.get(key) not in (None, ""):
            out[key] = str(raw[key]).strip()
    if raw.get("game_number") is not None:
        out["game_number"] = raw["game_number"]
    for key in ("pitcher_ids", "entity_ids"):
...
def normalize_offer_snapshot(data: Any, *, default_ttl_seconds: int = 300) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for i, raw in enumerate(data):
        try:
            ...
            key = (
                q["game_id"], q.get("event_id"), q.get("game_number"), q["period"],
                q["market"], q["entity_id"], repr(q["line"]), q["side"],
                q["book_key"], q["is_alternate"], q["american_odds"],
            )
            if key in seen:
