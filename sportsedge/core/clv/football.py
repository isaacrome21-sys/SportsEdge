...
def score_clv(decisions: Iterable[CLVDecision], closes: Iterable[CLVClose]) -> list[CLVScored]:
    close_index: dict[tuple[str, str, str, str], CLVClose] = {}
    for close in closes:
        key = (close.game_id, close.market, close.side, _book(close.book))
    ...
    for decision in decisions:
        exact_key = (decision.game_id, decision.market, decision.side, _book(decision.book))
        legacy_key = (decision.game_id, decision.market, decision.side, "")
        close = close_index.get(exact_key)
        if close is None and exact_key != legacy_key:
            close = close_index.get(legacy_key)
        if close is None:
            raise ValueError(f"CLOSE_MISSING:{exact_key}")
