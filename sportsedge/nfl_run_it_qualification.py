...
Game Picks materials motivate surfacing role, usage, matchup and availability
quality separately from price economics. This module adapts that disclosed idea
only. Sportsbook price, implied probability, edge and EV are forbidden inputs.
"""
...
def qualification_flags_from_snapshot(snapshot: Mapping[str, Any]) -> dict[str, bool]:
    """Build frozen Score-B flags from a pre-kickoff model/role snapshot.

    The caller must provide explicit upstream readiness decisions. We validate
    provenance and timing here; we do not infer quality from betting prices.
    """
    ...
    if forbidden:
        raise NflRunItQualificationError(
            "MARKET_INPUT_FORBIDDEN:" + ",".join(sorted(forbidden))
        )
