...
def _quote(row: Mapping[str, Any], *, game: GameSnapshot, provider_event: Mapping[str, Any], received_at: datetime, raw_sha256: str, source_uri: str, ttl_seconds: int) -> dict[str, Any]:
    market = str(row.get("market") or "")
    outcome = str(row.get("outcome") or "")
    base = {
        "game_id": str(game.game_pk), "period": "FG", "book_key": BOOK_KEY,
        "sportsbook": SPORTSBOOK, "retrieved_at": received_at,
        "ttl_seconds": int(ttl_seconds), "american_odds": int(row["price_american"]),
        "raw_market_name": f"DK_DIRECT:{market}", "is_alternate": False,
        "selection": outcome, "source": f"{SOURCE_CLASS};raw_sha256={raw_sha256}",
        "source_url": source_uri, "source_provider": SOURCE_CLASS,
...
def normalize_direct_dk_mlb_board(*, board: Any, schedule: Iterable[GameSnapshot], ttl_seconds: int = DEFAULT_TTL_SECONDS) -> DirectDKMLBSnapshot:
    ...
    for event_id, market in sorted(present_markets):
        ...
        if not rows:
            failures.append({"provider_event_id": event_id, "market": market, "stage": "MARKET_ADMISSION", "reason": "DirectDKMLBSourceError:DIRECT_DK_MARKET_PRESENT_BUT_NOT_ADMISSIBLE"})
            continue
        try:
            _validate_pair(market, rows, game)
            pair = [_quote(row, game=game, provider_event=event, received_at=received, raw_sha256=raw_sha, source_uri=str(board.source_uri), ttl_seconds=ttl_seconds) for row in rows]
            pending: list[tuple[str, str, str, str, float, int]] = []
            for quote in pair:
                identity = (quote["game_id"], quote["market"], quote["entity_id"], quote["side"], float(quote["line"]), int(quote["american_odds"]))
            ...
            failures.append({"provider_event_id": event_id, "market": market, "stage": "MARKET_ADMISSION", "reason": f"{type(exc).__name__}:{exc}"})
    return DirectDKMLBSnapshot(quotes=tuple(quotes), failures=tuple(failures), raw=raw, raw_sha256=raw_sha, source_uri=str(board.source_uri), received_at=received)


def fetch_mlb_direct_dk_game_quotes(*, target_date: date, now: datetime | None = None, board_fetcher: Callable[[str], Any] = fetch_board, schedule_opener: Callable = urlopen, ttl_seconds: int = DEFAULT_TTL_SECONDS) -> DirectDKMLBSnapshot:
