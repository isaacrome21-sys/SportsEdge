...
def _feature_index(rows):
    ...
    for row in rows:
        ...
        if market not in GENERIC_MARKETS or not game_id or not entity_id:
            continue
...
def _team_side(game, entity_id):
    ...
    if tid == int(game.away_team_id):
        return "AWAY"
    if tid == int(game.home_team_id):
        return "HOME"
...
def _bind_entity(game, market, entity_id, feature):
    ...
    if market in BATTER_GENERIC_MARKETS:
        live = _lineup_team(game, entity_id)
        ft = feature.get("team_id")
        if ft is not None and int(ft) != live:
    ...
    if market in PITCHER_GENERIC_MARKETS:
        if market.startswith("EITHER_PITCHER_"):
            if game.away_probable_pitcher_id is None or game.home_probable_pitcher_id is None:
                raise ValueError("BOTH_PROBABLE_PITCHERS_REQUIRED")
            expected = f"{int(game.away_probable_pitcher_id)}|{int(game.home_probable_pitcher_id)}"
            if entity_id != expected:
                raise ValueError("EITHER_PITCHER_CANONICAL_PAIR_MISMATCH")
            return
        try:
            pid = int(entity_id)
        except (TypeError, ValueError) as exc:
...
def _model_input(*, game, quote, feature):
    ...
    if market == "HOME_RUNS":
        ...
        payload = feature.get("features")
        if not isinstance(payload, Mapping):
            raise ValueError("PITCHER_RECORD_WIN state feature payload missing")
        out["features"] = dict(payload)
        out["team_side"] = feature.get("team_side")
        out["away_mean_runs"] = feature.get("away_mean_runs")
        out["home_mean_runs"] = feature.get("home_mean_runs")
...
def _candidate_economics(model_p: float, push_p: float, quote, opposite):
    p = float(model_p)
    push = float(push_p)
    if not isfinite(p) or not 0 <= p <= 1:
        raise ValueError("invalid Model_P")
    if not isfinite(push) or not 0 <= push < 1 or p + push > 1 + 1e-12:
        raise ValueError("invalid push probability")
    dec = american_to_decimal(quote["american_odds"])
    loss = max(0.0, 1.0 - p - push)
    ev = p * (dec - 1.0) - loss
    raw = 1.0 / dec
    settled_model_p = p / (1.0 - push)
    if opposite is None:
        return "MODEL_CANDIDATE_ONE_SIDED", None, raw, "UNAVAILABLE_ONE_SIDED", settled_model_p - raw, ev
    fair = multiplicative_devig(quote, opposite).candidate_fair_probability
    edge = settled_model_p - fair
    shadow = "SHADOW_BET" if edge > 0 and ev > 0 else "SHADOW_PASS"
    ...
    for raw in quotes:
        try:
            ...
            run = run_candidate(
                ...
                deployment={"market": market, **dict(deployment)},
                engine_fn=engine,
            ...
            if run.model_p is None or run.bet_status == "BLOCKED":
                results.append(GenericCardResult(
                    game_id=str(quote["game_id"]), market=market,
                    entity_id=str(quote["entity_id"]), line=quote["line"], side=str(quote["side"]),
                    american_odds=quote["american_odds"], model_p=None, bet_status="BLOCKED", reason=run.reason,
                ))
                continue
            if run.bet_status not in DECISION_STATUSES:
                raise RuntimeError(f"MODELED_ROW_WITHOUT_BET_PASS_DECISION: {run.bet_status}")
            shadow, fair, raw_p, novig_status, edge, ev = _candidate_economics(
                float(run.model_p), float(run.push_probability), quote, opposite
            )
            results.append(GenericCardResult(
                game_id=str(quote["game_id"]), market=market, entity_id=str(quote["entity_id"]),
                line=quote["line"], side=str(quote["side"]), american_odds=quote["american_odds"],
                model_p=float(run.model_p), bet_status=run.bet_status, reason=run.reason,
                shadow_status=shadow, implied_probability=fair, edge=edge, ev_per_dollar=ev,
                model_input_hash=run.model_input_hash, distribution_sha256=run.distribution_sha256,
                readout_sha256=run.readout_sha256, readout_version=run.readout_version,
                engine_version=run.engine_version, seed_policy=run.seed_policy, mc_paths=run.mc_paths,
                book_key=run.book_key, sportsbook=run.sportsbook,
                quote_retrieved_at=run.quote_retrieved_at, offer_id=run.offer_id,
                raw_implied_probability=raw_p, market_no_vig_p_status=novig_status,
            ))
        except Exception as exc:
            try:
                q = validate_canonical_quote(raw)
                identity = dict(
                    game_id=str(q["game_id"]), market=str(q["market"]), entity_id=str(q["entity_id"]),
                    line=q["line"], side=str(q["side"]), american_odds=q["american_odds"],
                )
            except Exception:
                identity = dict(game_id="UNKNOWN", market="UNKNOWN", entity_id="UNKNOWN", line=None, side="UNKNOWN", american_odds=None)
            results.append(GenericCardResult(**identity, model_p=None, bet_status="BLOCKED", reason=f"{type(exc).__name__}: {exc}"))
    return results
