...
This layer reuses SportsEdge's existing PIT settlement interpreters instead of
creating a second set of sportsbook semantics. It settles every MLB market for
which official facts + normalized book rules are already sufficient, and leaves
ambiguous markets UNRESOLVED rather than guessing.

A sportsbook identity is mandatory for a resolved prediction. The settlement
report freezes which sportsbook the validated rule records belong to, and the
prediction must match that identity before an outcome can be emitted.
"""
...
GAME_MARKETS = frozenset({"MONEYLINE", "RUN_LINE", "TOTALS"})
...
def _line(value: Any) -> float:
    if isinstance(value, bool):
        raise MLBCatalogSettlementError("prediction line must be finite numeric")
    try:
        out = float(value)
    except (TypeError, ValueError) as exc:
        raise MLBCatalogSettlementError("prediction line must be finite numeric") from exc
    if not isfinite(out):
        raise MLBCatalogSettlementError("prediction line must be finite numeric")
    return out
...
def _required_rules_by_market() -> dict[str, tuple[str, ...]]:
    ...
    return {
        str(row["market"]): tuple(str(x) for x in row["requirements"]["settlement_semantics"])
...
def _validated_sportsbook(required_rules: tuple[str, ...], evidence: Mapping[str, Any] | None) -> str | None:
    ...
    if not valid or not isinstance(evidence, Mapping):
        return None
    books = {
        normalize_name((evidence.get(rule) or {}).get("sportsbook"))
        for rule in required_rules
        if isinstance(evidence.get(rule), Mapping)
    }
    books.discard("")
    if len(books) != 1:
        return None
...
def build_catalog_settlement_report(
    facts: Mapping[str, Any],
    *,
    source: str,
    evidence_class: str = "LIVE_OFFICIAL_FACT_PROBE",
    generated_at_utc: str | None = None,
    book_rule_evidence: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Build settlement evidence while freezing sportsbook identity per market."""
    base = build_settlement_report(
        facts,
        source=source,
        evidence_class=evidence_class,
        generated_at_utc=generated_at_utc,
        book_rule_evidence=book_rule_evidence,
    )
    rules_by_market = _required_rules_by_market()
    rows: list[dict[str, Any]] = []
    for raw in base["markets"]:
        row = dict(raw)
        market = str(row["market"])
        sportsbook_identity = _validated_sportsbook(
            rules_by_market[market], book_rule_evidence
        )
        row["validated_sportsbook_normalized"] = sportsbook_identity
        rows.append(row)
    return {
        **base,
        "schema_version": CATALOG_REPORT_SCHEMA_VERSION,
        "markets": rows,
    }
...
def prediction_journal_sha256(record: Mapping[str, Any]) -> str:
    if not isinstance(record, Mapping):
        raise MLBCatalogSettlementError("prediction journal must be an object")
    if str(record.get("schema_version") or "") != JOURNAL_SCHEMA_VERSION:
        raise MLBCatalogSettlementError("unsupported prediction journal schema")
    predictions = record.get("predictions")
    if not isinstance(predictions, list):
        raise MLBCatalogSettlementError("prediction journal predictions must be a list")
    try:
        declared = int(record.get("prediction_count"))
    except (TypeError, ValueError) as exc:
        raise MLBCatalogSettlementError("prediction_count invalid") from exc
    if declared != len(predictions):
        raise MLBCatalogSettlementError("prediction_count mismatch")
...
def _report_game_id(report: Mapping[str, Any]) -> str:
    ...
    if not isinstance(facts, Mapping):
        raise MLBCatalogSettlementError("settlement facts missing")
    game_pk = str(facts.get("game_pk") or "").strip()
    if not game_pk:
        raise MLBCatalogSettlementError("settlement game_pk missing")
    return game_pk


def _report_index(reports: Mapping[str, Mapping[str, Any]]) -> dict[str, Mapping[str, Any]]:
    if not isinstance(reports, Mapping):
        raise MLBCatalogSettlementError("settlement_reports_by_game must be an object")
    out: dict[str, Mapping[str, Any]] = {}
    for supplied, report in reports.items():
        if not isinstance(report, Mapping):
            raise MLBCatalogSettlementError("settlement report must be an object")
        actual = _report_game_id(report)
        if str(supplied) != actual:
            raise MLBCatalogSettlementError(f"settlement report key/game mismatch: {supplied}!={actual}")
        if actual in out:
            raise MLBCatalogSettlementError(f"duplicate settlement report for game {actual}")
        out[actual] = report
    return out
...
def _market_row(report: Mapping[str, Any], market: str) -> Mapping[str, Any] | None:
    ...
    if not isinstance(rows, list):
        raise MLBCatalogSettlementError("settlement report markets must be a list")
    matches = [row for row in rows if isinstance(row, Mapping) and str(row.get("market") or "").upper() == market]
    if len(matches) > 1:
        raise MLBCatalogSettlementError(f"duplicate settlement market row: {market}")
...
def _prediction_sportsbook(prediction: Mapping[str, Any]) -> str | None:
    ...
    if sportsbook:
        return sportsbook
    book_key = normalize_name(prediction.get("book_key"))
    return book_key or None
...
def _eligibility(report: Mapping[str, Any], prediction: Mapping[str, Any], market: str) -> tuple[bool, str, str | None]:
    ...
    if str(report.get("schema_version") or "") != CATALOG_REPORT_SCHEMA_VERSION:
    ...
    if prediction_book is None:
        return False, "PREDICTION_SPORTSBOOK_IDENTITY_MISSING", evidence_sha
    validated_book = str(row.get("validated_sportsbook_normalized") or "").strip()
    if not validated_book:
        return False, "BOOK_RULE_SPORTSBOOK_IDENTITY_MISSING", evidence_sha
    if prediction_book != validated_book:
        return False, "BOOK_RULE_SPORTSBOOK_MISMATCH", evidence_sha
...
def _game_outcome(prediction: Mapping[str, Any], facts: Mapping[str, Any]) -> str:
    ...
    if not isinstance(game, Mapping):
        raise MLBCatalogSettlementError("final score missing")
    away = int(game["away_runs"])
    home = int(game["home_runs"])
    market = str(prediction.get("market") or "").upper()
    side = str(prediction.get("side") or "").upper()
    if market == "MONEYLINE":
        if away == home:
            raise MLBCatalogSettlementError("MONEYLINE_FINAL_TIE_REQUIRES_BOOK_RULE")
        if side in {"HOME", "HOME_ML"}:
            return "WIN" if home > away else "LOSS"
        if side in {"AWAY", "AWAY_ML"}:
    ...
    if market == "RUN_LINE":
        if side in {"HOME", "HOME_RL"}:
            ...
            raise MLBCatalogSettlementError("run-line side unsupported")
        if abs(margin) <= 1e-12:
            return "PUSH"
        return "WIN" if margin > 0 else "LOSS"
    if market == "TOTALS":
        total = away + home
        if abs(total - line) <= 1e-12:
            return "PUSH"
        if side == "OVER":
...
def _pitcher_value(facts: Mapping[str, Any], entity_id: str, market: str) -> float | None:
    field = _PITCHER_FIELD_BY_MARKET.get(market)
    rows = facts.get("pitchers")
    if field is None or not isinstance(rows, list):
        return None
...
def _resolve_prediction(prediction: Mapping[str, Any], facts: Mapping[str, Any]) -> tuple[str, str]:
    ...
    entity_id = str(prediction.get("entity_id") or "").strip()
    side = prediction.get("side")
    line = prediction.get("line")

    if market in GAME_MARKETS:
        return _game_outcome(prediction, facts), "OFFICIAL_GAME_FACTS_AND_NORMALIZED_BOOK_RULES"

    if market in ADDITIONAL_MARKETS:
        ambiguity = _policy_ambiguity(facts, market)
        if ambiguity:
            return "UNRESOLVED", str(ambiguity[0])
        try:
            result = _derive_additional_outcome(facts, market, entity_id, line, side)
        except Exception as exc:
            return "UNRESOLVED", f"ADDITIONAL_SETTLEMENT_ERROR:{exc}"
        return result, "OFFICIAL_MARKET_FACTS_AND_NORMALIZED_BOOK_RULES"

    if market in EITHER_PITCHER_MARKETS:
        # The provider proposition is a conjunction/disjunction over two pitcher
        # identities. The existing PIT lane intentionally refuses to guess its
        # exact threshold semantics; keep the immutable prediction lane aligned.
        return "UNRESOLVED", "EITHER_PITCHER_SETTLEMENT_INTERPRETER_REQUIRED"

    value = _fact_value(facts, market, entity_id)
    if value is None:
        # Explicit fallback for ordinary pitcher markets to keep the failure
        # reason narrow when a player row is absent from official facts.
        value = _pitcher_value(facts, entity_id, market)
    if value is None:
        return "UNRESOLVED", "OFFICIAL_ENTITY_FACT_VALUE_MISSING"
    try:
        result = _derive_count_outcome(value=value, line=line, side=side)
    except Exception as exc:
        return "UNRESOLVED", f"COUNT_SETTLEMENT_ERROR:{exc}"
    return result, "OFFICIAL_COUNT_FACT_AND_NORMALIZED_BOOK_RULES"
...
def build_catalog_prediction_settlement_record(
    ...
    *,
    ...
    if expected_prediction_journal_sha256 is not None:
        expected = _valid_sha256(expected_prediction_journal_sha256, "expected_prediction_journal_sha256")
    ...
    for index, raw in enumerate(predictions):
        if not isinstance(raw, Mapping):
            raise MLBCatalogSettlementError(f"prediction[{index}] must be an object")
        market = str(raw.get("market") or "").strip().upper()
        game_id = str(raw.get("game_id") or "").strip()
        if not market or not game_id:
            raise MLBCatalogSettlementError(f"prediction[{index}] identity incomplete")
        report = reports.get(game_id)
        if report is None:
            outcomes.append(_unresolved(raw, "GAME_SETTLEMENT_EVIDENCE_MISSING"))
            continue
        eligible, reason, evidence_sha = _eligibility(report, raw, market)
        if not eligible:
            outcomes.append(_unresolved(raw, reason, evidence_sha))
            continue
        facts = report.get("facts")
        assert isinstance(facts, Mapping)
        result, result_reason = _resolve_prediction(raw, facts)
        outcomes.append({
            **dict(raw),
