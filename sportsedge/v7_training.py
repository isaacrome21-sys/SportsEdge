...
def _fit_standardized_logit(
    x: Sequence[Sequence[float]], y: Sequence[int], *, l2: float, iterations: int, learning_rate: float
) -> tuple[float, list[float]]:
    if not x or len(x) != len(y):
        raise V7TrainingError("non-empty paired training matrix required")
    n = len(x)
    p = len(x[0])
    if p == 0 or any(len(row) != p for row in x):
        raise V7TrainingError("invalid training matrix shape")
    if n < 10:
        raise V7TrainingError("at least 10 training rows required")
    if not 0 <= float(l2) < 1e6 or int(iterations) <= 0 or not 0 < float(learning_rate) <= 1:
        raise V7TrainingError("invalid optimizer settings")
    ...
    for _ in range(int(iterations)):
        errors: list[float] = []
        for row, target in zip(zmat, y):
            score = intercept + sum(b * v for b, v in zip(beta, row))
            errors.append(_sigmoid(score) - target)
        grad_i = sum(errors) / n
...
def _calibration_delta(logits: Sequence[float], outcomes: Sequence[int]) -> float:
    if not logits or len(logits) != len(outcomes):
        raise V7TrainingError("paired calibration rows required")
...
def train_chronological_candidate(
    rows: Iterable[Mapping[str, Any]], *, model_name: str,
    ...
    cal_start_d = _date(calibration_start, "calibration_start")
    cal_end_d = _date(calibration_end, "calibration_end")
    ...
    if any(_date(r.get(date_key), date_key) >= holdout_start_d for r in train_rows + cal_rows):
        raise V7TrainingError("holdout contamination detected")
    if len(cal_rows) < 2:
    ...
    base_report = {
        "training_version": V7_TRAINING_VERSION,
        "model_name": str(model_name),
        "train_n": len(train_rows),
        "calibration_n": len(cal_rows),
        "train_end": train_end_d.isoformat(),
        "calibration_start": cal_start_d.isoformat(),
        "calibration_end": cal_end_d.isoformat(),
    ...
    report_hash = canonical_json_sha256(base_report)
...
def evaluate_frozen_candidate(
    artifact: Mapping[str, Any], rows: Iterable[Mapping[str, Any]], *, label_key: str = "outcome", name: str = "holdout"
) -> dict[str, Any]:
    candidate = load_candidate(artifact)
    material = list(rows)
