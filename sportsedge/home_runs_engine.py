"""Dedicated research/shadow MLB batter home-run engine.

This engine is intentionally price-independent and remains ineligible for official
betting until the repo-wide validation gates pass.  It uses only pregame baseball
features.  Statcast contact-quality fields are required and hashed for provenance,
but are not assigned fitted coefficients until a historical point-in-time training
and untouched-holdout fit freezes them.
"""
...
def simulate_home_runs(model_input: Mapping[str, Any], thresholds: tuple[float, ...] = (0.5,)) -> HomeRunsModelOutput:
    ...
    for raw_threshold in thresholds:
        ...
        # Probability of at least one HR, averaged over the empirical PA pool.
