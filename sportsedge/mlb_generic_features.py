...
The builder consumes only official MLB game-log history dated strictly before the
target game. It intentionally produces a transparent baseline contract for shadow
recommendations; it does not confer production validation or deployment status.
Sportsbook price/probability fields are never inputs.
"""
...
class MLBGenericHistorySource:
    ...
    def batter_expected(self, *, player_id: int, market: str, target_date: date) -> float:
        ...
        for row in rows:
            ...
            if market == "HITS_RUNS_RBIS":
                value = _number(s.get("hits", 0), "hits") + _number(s.get("runs", 0), "runs") + _number(s.get("rbi", 0), "rbi")
            elif market == "SINGLES":
                value = _number(s.get("hits", 0), "hits") - _number(s.get("doubles", 0), "doubles") - _number(s.get("triples", 0), "triples") - _number(s.get("homeRuns", 0), "homeRuns")
                value = max(0.0, value)
            else:
    ...
    def team_means(self, *, away_team_id: int, home_team_id: int, target_date: date) -> tuple[float, float, float]:
        def one(team_id: int) -> tuple[float, float]:
            ...
            return (
                self._mean(runs, minimum=10, window=30, name=f"team:{team_id}:runs"),
                self._mean(hrs, minimum=10, window=30, name=f"team:{team_id}:hr"),
            )
        ...
        *,
        ...
        if market in PLAYER_COUNT_MARKETS:
            ...
            if market.startswith("PITCHER_"):
                expected = self.pitcher_expected(player_id=player_id, market=market, target_date=target_date)
            else:
                expected = self.batter_expected(player_id=player_id, market=market, target_date=target_date)
