...
def board_payload(*, drop_under=False, mismatch_spread=False, drop_spread_away=False, bad_event_name=False):
    sels = [
        {"marketId": "m1", "label": "Chicago Bears", "points": -3.5, "displayOdds": {"american": "-110"}},
        {"marketId": "m1", "label": "Green Bay Packers", "points": 3.5 if not mismatch_spread else 4.5, "displayOdds": {"american": "-110"}},
        {"marketId": "m2", "label": "Over 44.5", "points": 44.5, "displayOdds": {"american": "-105"}},
        {"marketId": "m2", "label": "Under 44.5", "points": 44.5, "displayOdds": {"american": "-115"}},
    ]
    if drop_under:
    ...
    return {
        "events": [{"id": "E1", "name": "Green Bay Packers vs Chicago Bears" if bad_event_name else "Green Bay Packers @ Chicago Bears", "startEventDate": "2026-09-20T17:00:00Z"}],
        "markets": [{"id": "m1", "eventId": "E1", "name": "Spread"}, {"id": "m2", "eventId": "E1", "name": "Total"}],
        "selections": sels,
    }
...
class DirectCaptureAdapterTest(unittest.TestCase):
    def _transport(self, payload=None, **kw):
        ...
        def _fetch(sk):
            return RawDraftKingsBoard(sk, src.board_url(sk), raw, datetime(2026, 9, 22, 14, 0, 5, tzinfo=UTC), payload)
    ...
    def test_transport_record_preserves_exact_raw_and_provenance(self):
        ...
        self.assertEqual(t["attempts"][0]["result_class"], "OK")
    ...
    def test_window_is_end_exclusive(self):
        t = self._transport()
        self.assertEqual(src.game_rows_direct(t, week_of=week_of, window_start=datetime(2026, 9, 20, 0, 0, tzinfo=UTC), window_end=datetime(2026, 9, 20, 17, 0, tzinfo=UTC)), [])
    ...
    def test_failure_records_attempt_history(self):
        def boom(_sk): raise DraftKingsGameMarketError("DK_GAME_HTTP_403")
