...
def test_bound_score_is_quality_not_probability_or_ev():
    ...
    assert out.ev > 0
    assert out.score == round(100*(.8*.9)**.5)
    assert out.score_version == "NBA_RUN_IT_SCORE_RULE_B_V2"


def test_quality_controls_score_but_odds_and_ev_do_not():
    now=datetime(2026,1,1,tzinfo=timezone.utc)
    fair=path_outcome((25,26,27,20),24.5,over=True)
    ...
    rich=bind_quote(quote(now,odds=3.0),fair,as_of=now,model_quality=.8,context_quality=.9)
    poor=bind_quote(quote(now,odds=1.5),fair,as_of=now,model_quality=.8,context_quality=.9)
    assert rich.ev != poor.ev
