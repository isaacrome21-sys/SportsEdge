...
def row(*, player="00-0030001", team="CHI", week=1, modified="2026-09-01T13:00:00Z",
    ...
    return {
        "season": "2026", "season_type": "REG", "team": team, "week": str(week),
        "gsis_id": player, "position": "WR", "full_name": "Example Player",
        "report_primary_injury": "Hamstring", "report_secondary_injury": "",
        "report_status": report_status, "practice_primary_injury": "Hamstring",
        "practice_secondary_injury": "", "practice_status": practice_status,
...
class NFLInjuryReportSourceTests(unittest.TestCase):
    def test_latest_pre_pit_row_wins_and_post_pit_update_is_excluded(self):
        ...
        out = build_pit_injury_inputs(rows=rows, season=2026, target_week=1, team_ids=("CHI", "GB"), as_of=PIT, source_uri=URI, source_sha256=SHA)
    ...
    def test_wrong_team_week_and_missing_timestamp_are_surgically_excluded(self):
        rows = [row(team="DAL"), row(week=2), row(player="00-0030002", modified=""), row(player="00-0030003", team="GB", modified="2026-09-01T12:30:00Z")]
        out = build_pit_injury_inputs(rows=rows, season=2026, target_week=1, team_ids=("CHI", "GB"), as_of=PIT, source_uri=URI, source_sha256=SHA)
        self.assertEqual([item["player_id"] for item in out], ["00-0030003"])

    def test_near_official_binding_never_claims_official_host(self):
        injury = build_pit_injury_inputs(rows=[row()], season=2026, target_week=1, team_ids=("CHI", "GB"), as_of=PIT, source_uri=URI, source_sha256=SHA)
        bundle = {"collection_mode": "AUTO", "as_of_utc": PIT.isoformat(), "observations": {"injury_availability": {"status": "MISSING", "source_name": "NFL_OBJECTIVE_PROVIDER"}}}
    ...
    def test_existing_official_available_observation_has_precedence(self):
        bundle = {"collection_mode": "AUTO", "observations": {"injury_availability": {"status": "AVAILABLE", "source_name": "OFFICIAL_NFL_INJURY_REPORTS", "payload": [{"status": "OUT"}]}}}
        injury = build_pit_injury_inputs(rows=[row()], season=2026, target_week=1, team_ids=("CHI", "GB"), as_of=PIT, source_uri=URI, source_sha256=SHA)
