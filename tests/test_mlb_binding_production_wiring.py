...
def game(game_number=1):
    return LiveGame(
        ...
        away_lineup=TeamLineup(20, "away", (), (), False),
        home_lineup=TeamLineup(10, "home", (), (), False),
...
def fake_run(model_p=0.55, push=0.0):
    return SimpleNamespace(
        model_p=model_p,
        bet_status="PASS",
        reason="TEST_PASS",
        decision=SimpleNamespace(push_probability=push),
        model_input_hash="mi",
        distribution_sha256="dist",
        readout_sha256="ro",
        readout_version="v",
        engine_version="e",
        seed_policy="seed",
...
class ProductionWiringTests(unittest.TestCase):
    def _run(self, quotes, features, *, live_game=None, run=None):
        ...
        with patch("sportsedge.generic_card_pipeline.load_registry", return_value={"markets": {m: {} for m in markets}}), patch("sportsedge.generic_card_pipeline.engine_registry", return_value={m: object() for m in markets}), patch("sportsedge.generic_card_pipeline.run_candidate", return_value=run or fake_run()):
            return run_generic_card(
                games=[live_game or game()],
    ...
    def test_all_four_wired_game_families_traverse_quote_bound_pipeline(self):
        ...
        for quotes, features in cases:
            with self.subTest(market=quotes[0]["market"]):
                out = self._run(quotes, features)
    ...
    def test_same_signed_runline_pair_blocks(self):
        quotes = [quote("RUN_LINE", 10, "HOME", line=-1.5), quote("RUN_LINE", 20, "AWAY", line=-1.5)]
        out = self._run(quotes, [feature("RUN_LINE", 10), feature("RUN_LINE", 20)])
    ...
    def test_team_total_different_team_cannot_pair(self):
        quotes = [quote("TEAM_TOTALS", 10, "OVER", line=4.5), quote("TEAM_TOTALS", 20, "UNDER", line=4.5)]
        out = self._run(quotes, [feature("TEAM_TOTALS", 10), feature("TEAM_TOTALS", 20)])
        self.assertTrue(all(r.bet_status == "BLOCKED" for r in out))

    def test_missing_official_game_number_is_per_market_block(self):
        quotes = [quote("TOTALS", 777, "OVER", line=8.5), quote("TOTALS", 777, "UNDER", line=8.5)]
        out = self._run(quotes, [feature("TOTALS", 777)], live_game=game(None))
        self.assertTrue(all(r.bet_status == "BLOCKED" for r in out))
    ...
    def test_post_model_full_binding_is_called(self):
        ...
        with patch("sportsedge.generic_card_pipeline.validate_binding", wraps=real_validate_binding) as spy, patch("sportsedge.generic_card_pipeline.load_registry", return_value={"markets": {"TOTALS": {}}}), patch("sportsedge.generic_card_pipeline.engine_registry", return_value={"TOTALS": object()}), patch("sportsedge.generic_card_pipeline.run_candidate", return_value=fake_run()):
            out = run_generic_card(
                games=[game()],
                feature_rows=[feature("TOTALS", 777)],
                quotes=quotes,
                ingestion_now=NOW,
                finalization_now=NOW,
            )
    ...
    def test_wrong_probability_event_blocks_post_model(self):
        ...
        def corrupt(**kwargs):
            result = real_probability_binding_row(**kwargs)
            result["probability_event_id"] = "wrong-event"
            return result

        with patch("sportsedge.generic_card_pipeline.probability_binding_row", side_effect=corrupt):
            out = self._run(quotes, [feature("TOTALS", 777)])
        self.assertTrue(all(r.bet_status == "BLOCKED" for r in out))
    ...
    def test_wrong_probability_side_blocks_post_model(self):
        ...
        def corrupt(**kwargs):
            result = real_probability_binding_row(**kwargs)
            result["probability_side"] = "UNDER" if result["side"] == "OVER" else "OVER"
            return result
    ...
    def test_external_binding_mode_requires_boundary_attestation(self):
        candidate = quote("TOTALS", 777, "OVER", line=8.5)
        ...
        result = run_candidate(
            ...
            deployment={},
            engine_fn=lambda _: {"model_p": 0.55},
            ingestion_now=NOW,
            finalization_now=NOW,
    ...
    def test_external_binding_mode_never_calls_legacy_candidate_binder(self):
        ...
        opposite = quote("TOTALS", 777, "UNDER", line=8.5)
        model_input = {"game_id": "777", "market": "TOTALS", "entity_id": "777", "line": 8.5, "side": "OVER", "away_mean_runs": 4.0, "home_mean_runs": 4.5}
        attestation = validate_normalized_mlb_quote_binding(game=game(), quote=candidate)
    ...
    def test_engine_cannot_smuggle_source_binding_identity(self):
        ...
        result = run_candidate(
            ...
            paired_quote=opposite,
            deployment={},
            engine_fn=lambda _: {"model_p": 0.55, "event_id": "777"},
            ingestion_now=NOW,
            finalization_now=NOW,
            candidate_binding_mode=MLB_EXTERNAL_BINDING_MODE,
            binding_attestation=attestation,
