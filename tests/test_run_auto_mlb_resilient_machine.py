...
class ResilientMachineRoutingTests(unittest.TestCase):
    def test_native_production_calls_run_it_automatic_and_ignores_legacy_feature_url(self):
    ...
    def test_external_quote_url_remains_explicit_legacy_lane(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            legacy = type("Legacy", (), {
                ...
                "generated_at_utc": "2026-08-24T14:30:00+00:00",
                "run_status": "PASS",
                "results": (machine_report().results[0],),
                "source_failures": (),
            })()
            env = {
            ...
            with patch.dict(os.environ, env, clear=True), \
                 patch.object(sys, "argv", [str(SCRIPT), "--output", str(output)]), \
                 patch.object(mod, "run_auto_mlb", return_value=legacy) as legacy_runner, \
