...
def _game(game_pk: int, day: int, away_f5: int, home_f5: int):
    # Put all F5 scoring in inning 1 and then add a deliberately large late
    # score. A 5/9 final-score shortcut would disagree with the asserted F5
    # histories below.
    innings = [
...
class MLBGenericF5FeatureTests(unittest.TestCase):
    def setUp(self):
        ...
        # Deliberately reverse provider order. The feature builder must restore
        # chronological ordering before keeping its trailing history window.
    ...
    def test_f5_features_use_actual_strict_prior_innings_not_final_score_scaling(self):
        ...
        # The row is immediately consumable by the canonical shared F5 engine
        # distribution contract (minimum ten aligned strictly-prior games).
