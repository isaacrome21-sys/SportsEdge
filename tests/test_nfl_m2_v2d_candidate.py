...
def test_v2d_fits_direct_scores_and_replays_coherent_integer_paths():
    rows = _rows()
    model = fit_nfl_m2_v2d_candidate(rows, ridge_alpha=10.0)
    assert model.model_id == NFL_M2_V2D_CANDIDATE_MODEL_ID
    assert model.train_seasons == (2020, 2021, 2022)
    paths = derive_nfl_m2_v2d_score_distribution(model, rows[-1])
    assert len(paths) == len(rows)
    assert all(isinstance(path["home_score"], int) for path in paths)
    assert all(isinstance(path["away_score"], int) for path in paths)
    assert all(path["home_score"] >= 0 and path["away_score"] >= 0 for path in paths)


def test_v2d_market_fields_are_rejected_not_features():
    rows = _rows()
    contaminated = deepcopy(rows)
    contaminated[0]["home_features"]["spread"] = -3.0
    with pytest.raises(ValueError, match="M2_MARKET_DATA_PROHIBITED"):
        fit_nfl_m2_v2d_candidate(contaminated)


def test_v2d_unseen_heldout_outcome_cannot_change_training_fit():
    train = _rows()
    heldout = _row(2023, 2.0, 10, 40)
    model_a = fit_nfl_m2_v2d_candidate(train)
    mutated = deepcopy(heldout)
    mutated["home_score"] = 70
    mutated["away_score"] = 0
    # Held-out outcome is not supplied to fit; changing it cannot alter the model.
    model_b = fit_nfl_m2_v2d_candidate(train)
    assert model_a == model_b
    assert derive_nfl_m2_v2d_score_distribution(model_a, heldout) == derive_nfl_m2_v2d_score_distribution(model_b, mutated)


def test_v2d_is_deterministic():
    rows = _rows()
