...
from sportsedge.mlb_catalog_prediction_settlement import (
    build_catalog_prediction_settlement_record,
    build_catalog_settlement_report,
)
...
def _rules(sportsbook="DraftKings"):
    ...
    for row in build_acceptance_matrix()["markets"]:
        required.update(row["requirements"]["settlement_semantics"])
    return {
        rule: {
            "status": "VALIDATED",
            "sportsbook": sportsbook,
            "source_sha256": SHA,
            "captured_at_utc": "2026-08-25T11:30:00+00:00",
            "source_locator": f"fixture://{sportsbook}/{rule}",
        }
...
def _facts():
    return {
        ...
        "first_home_run": {"occurred": True, "batter_id": "10"},
        "winning_pitcher": {"pitcher_id": "20"},
    }
...
def _prediction(market, entity, side, line, *, sportsbook="DraftKings"):
    ...
    if sportsbook is not None:
        ...
        row["book_key"] = sportsbook.lower().replace(" ", "")
...
def _report(*, sportsbook="DraftKings", evidence_class="LIVE_OFFICIAL_FACT_PROBE"):
    return build_catalog_settlement_report(
        _facts(),
        source="MLB_STATSAPI_BOX_SCORE_AND_LIVE_FEED",
        evidence_class=evidence_class,
        generated_at_utc="2026-08-25T12:00:00+00:00",
        book_rule_evidence=_rules(sportsbook),
    )


class MLBCatalogPredictionSettlementTests(unittest.TestCase):
    def test_resolves_representative_market_families_from_one_official_fact_report(self):
        journal = _journal(
            _prediction("MONEYLINE", GAME_ID, "HOME", None),
            _prediction("PITCHER_OUTS", "20", "OVER", 15.5),
            _prediction("PITCHER_K", "20", "OVER", 5.5),
            _prediction("HOME_RUNS", "10", "OVER", 0.5),
            _prediction("F5_TOTALS", GAME_ID, "OVER", 2.5),
            _prediction("NRFI", GAME_ID, "YES", 0.0),
            _prediction("FIRST_HOME_RUN", "10", "YES", 0.0),
            _prediction("PITCHER_RECORD_WIN", "20", "YES", 0.0),
            _prediction("HITS", "10", "OVER", 1.5),
            _prediction("TOTAL_BASES", "10", "OVER", 3.5),
            _prediction("PITCHER_BB", "20", "UNDER", 2.5),
        )
        record = build_catalog_prediction_settlement_record(
            journal,
            settlement_reports_by_game={GAME_ID: _report()},
            settled_at_utc="2026-08-25T12:10:00+00:00",
        )
        self.assertIsNotNone(record)
        self.assertEqual(record["prediction_count"], 11)
        self.assertEqual(record["resolved_count"], 11)
    ...
    def test_prediction_sportsbook_must_match_validated_rule_sportsbook(self):
        ...
        record = build_catalog_prediction_settlement_record(
            journal,
            settlement_reports_by_game={GAME_ID: _report(sportsbook="DraftKings")},
            settled_at_utc="2026-08-25T12:10:00+00:00",
        )
        row = record["outcomes"][0]
        self.assertEqual(row["settlement_result"], "UNRESOLVED")
        self.assertEqual(row["settlement_reason"], "BOOK_RULE_SPORTSBOOK_MISMATCH")
    ...
    def test_synthetic_evidence_never_resolves_real_prediction(self):
        ...
        record = build_catalog_prediction_settlement_record(
            journal,
            settlement_reports_by_game={GAME_ID: _report(evidence_class="SYNTHETIC_CONTRACT_TEST")},
            settled_at_utc="2026-08-25T12:10:00+00:00",
        )
    ...
    def test_either_pitcher_family_remains_unresolved_until_interpreter_is_normalized(self):
        ...
        row = record["outcomes"][0]
        self.assertEqual(row["settlement_result"], "UNRESOLVED")
        self.assertEqual(row["settlement_reason"], "EITHER_PITCHER_SETTLEMENT_INTERPRETER_REQUIRED")

    def test_first_home_run_no_hr_game_preserves_policy_ambiguity(self):
        facts = _facts()
        facts["first_home_run"] = {"occurred": False}
        report = build_catalog_settlement_report(
            facts,
            source="MLB_STATSAPI_BOX_SCORE_AND_LIVE_FEED",
            generated_at_utc="2026-08-25T12:00:00+00:00",
            book_rule_evidence=_rules(),
        )
