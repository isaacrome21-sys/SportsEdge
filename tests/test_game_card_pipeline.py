...
class Tests(unittest.TestCase):
    def setUp(self):
        self.now=datetime(2026,8,12,12,1,tzinfo=timezone.utc)
        self.rows=[{'game_id':'123','run_rows':[[4.4,4.4,4.4,4.4,4.4,0,4,0,0,1],[4.4,4.4,4.4,4.4,4.4,1,4,0,0,1]],'fi_row':[.28]*13+[4,4,0,1]}]

    def test_legacy_v4_cannot_reach_any_statcast_required_game_model_path(self):
        game,nrfi=legacy_v4_artifacts()
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out=run_game_card(feature_rows=self.rows,quotes=qs,game_score_artifact=game,nrfi_artifact=nrfi,ingestion_now=self.now,finalization_now=self.now,registry_path=str(reg))
        ...
        self.assertTrue(all('STATCAST_' in x.reason for x in out))

    def test_fresh_legacy_v4_is_blocked_but_stale_price_still_fails_before_model(self):
        game,nrfi=legacy_v4_artifacts()
        fresh=run_game_card(feature_rows=self.rows,quotes=[quote('MONEYLINE','HOME',None)],game_score_artifact=game,nrfi_artifact=nrfi,ingestion_now=self.now,finalization_now=self.now)
        self.assertEqual(fresh[0].bet_status,'BLOCKED'); self.assertIsNone(fresh[0].model_p); self.assertIn('STATCAST_',fresh[0].reason)
        stale_quote=quote('MONEYLINE','HOME',None,retrieved_at=self.now-timedelta(seconds=601))
        stale=run_game_card(feature_rows=self.rows,quotes=[stale_quote],game_score_artifact=game,nrfi_artifact=nrfi,ingestion_now=self.now,finalization_now=self.now)
