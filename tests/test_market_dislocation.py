...
from sportsedge.market_dislocation import (
    MarketDislocationError,
    scan_market_dislocation,
)
...
class MarketDislocationTests(unittest.TestCase):
    ...
    def test_does_not_treat_raw_single_book_probability_as_fair(self):
        candidate = ml("draftkings", "HOME", 20, +110)
        # Only one side exists at each reference book, so no de-vigged fair price can be formed.
    ...
    def test_integer_total_reports_conditional_ev_but_not_fake_exact_ev(self):
