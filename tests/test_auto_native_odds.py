...
def event_odds(player="Cubs Batter 1", market="batter_hits"):
    return {"id": "provider-event","bookmakers": [{"key": "draftkings","title": "DraftKings","last_update": "2026-08-11T14:59:00Z","markets": [{"key": market,"outcomes": [{"name": "Over", "description": player, "point": 0.5, "price": -110, "sid": "over-1"},{"name": "Under", "description": player, "point": 0.5, "price": -110, "sid": "under-1"}]}]}]}
...
def batter_history():
    start = date(2026, 4, 1); rows = []
    for i in range(25):
        day = (start + timedelta(days=i)).isoformat(); rows.append(hrow(day, 500+i, 2, 5, d=1 if i % 4 == 0 else 0, t=1 if i == 3 else 0, hr=1 if i % 7 == 0 else 0))
    return rows

def fielding_history():
    start = date(2026, 4, 1); return [frow((start + timedelta(days=i)).isoformat(), 500+i, 1, "DH" if i % 6 == 0 else "1B") for i in range(25)]
def pitching_history():
    start = date(2026, 4, 1); return [prow((start + timedelta(days=7*i)).isoformat(), 600+i, 5, 25, hr=1) for i in range(5)]
...
class AutoNativeOddsTests(unittest.TestCase):
    ...
    def native_opener(self, req, timeout=15, *, market="batter_hits", venue_unmapped=False):
        ...
        if "statsapi.mlb.com/api/v1/schedule?" in url:
            value = schedule()
            if venue_unmapped: value["dates"][0]["games"][0]["venue"]["id"] = 2529
            return Resp(value)
        if "statsapi.mlb.com/api/v1/game/777/boxscore" in url: return Resp(boxscore())
        if "api.the-odds-api.com/v4/sports/baseball_mlb/events?" in url: return Resp(events())
        if "api.the-odds-api.com/v4/sports/baseball_mlb/events/provider-event/odds?" in url: return Resp(event_odds(market=market))
        if "/people/" in url and any(f"season={year}" in url for year in range(2021, 2026)): return Resp(stat_payload([]))
    ...
    def test_native_odds_path_binds_provider_player_to_exact_mlb_id(self):
        ...
        self.assertEqual(report.slate_date_ct, "2026-08-11"); self.assertEqual(len(report.results), 2); self.assertEqual({r.entity_id for r in report.results}, {"100"}); self.assertEqual({r.market for r in report.results}, {"HITS"}); self.assertTrue(all(r.bet_status != "OFFICIAL_BET" for r in report.results))

    def test_native_auto_fetches_provider_event_list_once_per_key_attempt(self):
        calls = {"events": 0}
        def op(req, timeout=15):
            url = req if isinstance(req, str) else req.full_url
            if "api.the-odds-api.com/v4/sports/baseball_mlb/events?" in url: calls["events"] += 1
            return self.opener(req, timeout)
        run_auto_mlb_native_odds(odds_api_key="secret",feature_url="https://features",now=NOW,opener=op)
    ...
    def test_unknown_provider_player_is_not_guessed(self):
        ...
        self.assertEqual(report.results, ()); self.assertEqual(report.run_status, "DEGRADED"); self.assertTrue(any("ODDS_PLAYER_ID_UNRESOLVED" in str(x) for x in report.source_failures))
    ...
    def test_no_feature_url_builds_hits_features_from_official_history(self):
        ...
        self.assertEqual(len(report.results), 2); self.assertEqual({r.market for r in report.results}, {"HITS"})
        self.assertTrue(all(r.bet_status == "MODEL_CANDIDATE" for r in report.results)); self.assertTrue(all(r.model_p is not None for r in report.results)); self.assertTrue(all("feature snapshot missing" not in r.reason.lower() for r in report.results)); self.assertTrue(all("official_blocked" in r.reason.lower() for r in report.results)); self.assertFalse(any(x.get("stage") == "MLB_HITS_FEATURE" for x in report.source_failures))
    ...
    def test_no_feature_url_builds_total_bases_features_from_official_history(self):
        ...
        self.assertEqual(len(report.results), 2); self.assertEqual({r.market for r in report.results}, {"TOTAL_BASES"})
        self.assertTrue(all(r.bet_status == "MODEL_CANDIDATE" for r in report.results)); self.assertTrue(all(r.model_p is not None for r in report.results)); self.assertTrue(all("feature snapshot missing" not in r.reason.lower() for r in report.results)); self.assertTrue(all("official_blocked" in r.reason.lower() for r in report.results)); self.assertFalse(any(x.get("stage") == "MLB_TOTAL_BASES_FEATURE" for x in report.source_failures))
    ...
    def test_unmapped_venue_blocks_total_bases_instead_of_using_neutral_park(self):
        ...
        report = run_auto_mlb_native_odds(odds_api_key="secret", feature_url=None, now=NOW, opener=op)
        self.assertEqual(len(report.results), 2); self.assertTrue(all(r.bet_status == "BLOCKED" for r in report.results)); self.assertTrue(any("VENUE_UNMAPPED" in str(x) for x in report.source_failures))
