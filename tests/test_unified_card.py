...
def quote(market, entity, line=.5):
    return {"game_id":"777","market":market,"entity_id":str(entity),"line":line,"side":"OVER","american_odds":100,"retrieved_at":NOW,"ttl_seconds":300}
...
def registry(path):
    path.write_text(json.dumps({"schema_version":1,"markets":{
        "HITS":{"eligible":True,"stage":"DEPLOYED","reason":"isolation-test"},
        "TOTAL_BASES":{"eligible":True,"stage":"DEPLOYED","reason":"isolation-test"},
        "PITCHER_BB":{"eligible":True,"stage":"DEPLOYED","reason":"isolation-test"},
    }}))


def key(x):
    return (x.market, x.entity_id, float(x.line), x.side)
...
class UnifiedCardIsolationTests(unittest.TestCase):
    def test_unified_matches_each_standalone_pipeline_exactly(self):
        ...
        for x in unified:
            y = standalone[key(x)]
            self.assertEqual(x.model_p, y.model_p)
            self.assertEqual(x.bet_status, y.bet_status)
    ...
    def test_checked_registry_keeps_all_three_blocked_independently(self):
        ...
        self.assertEqual([x.market for x in out], ["HITS","TOTAL_BASES","PITCHER_BB"])
        self.assertTrue(all(x.bet_status == "BLOCKED" for x in out))

    def test_unsupported_market_is_explicit_blocked_row_and_does_not_change_hits(self):
        bad = quote("FOO", 100, .5)
        with tempfile.TemporaryDirectory() as td:
            p = Path(td)/"deployments.json"; registry(p)
            a = run_unified_card(games=[game()], feature_rows=features(), quotes=[all_quotes()[0]], ingestion_now=NOW, finalization_now=NOW, registry_path=str(p))
            b = run_unified_card(games=[game()], feature_rows=features(), quotes=[bad, all_quotes()[0]], ingestion_now=NOW, finalization_now=NOW, registry_path=str(p))
        self.assertEqual(b[0].bet_status, "BLOCKED")
        self.assertIn("unsupported unified market", b[0].reason)
        self.assertEqual(a[0].model_p, b[1].model_p)
