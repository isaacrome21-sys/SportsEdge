...
class SpecTests(unittest.TestCase):
    ...
    def test_every_market_has_explicit_threshold_domain(self):
...
class FullBindingTests(unittest.TestCase):
    def test_probability_team_mismatch_fails(self):
    ...
    def test_orchestrator_executes_binding_for_first_four(self):
        cases = [
            ("MONEYLINE", q("MONEYLINE", "HOME", 20, 0.0, -120), q("MONEYLINE", "AWAY", 10, 0.0, +110), 0.56),
            ("RUN_LINE", q("RUN_LINE", "HOME", 20, -1.5, +105), q("RUN_LINE", "AWAY", 10, +1.5, -125), 0.51),
            ("TOTALS", q("TOTALS", "OVER", 777, 8.5, -110), q("TOTALS", "UNDER", 777, 8.5, -110), 0.54),
            ("TEAM_TOTALS", q("TEAM_TOTALS", "OVER", 20, 4.5, -110), q("TEAM_TOTALS", "UNDER", 20, 4.5, -110), 0.54),
        ]
        ...
        with patch("sportsedge.orchestrator.require_production_edge_floor", return_value=floor):
            for market, quote, opposite, p in cases:
                ...
                result = run_candidate(
                    ...
                    engine_fn=lambda _mi, p=p: {"model_p": p, "push_p": 0.0},
    ...
    def test_binding_failure_is_row_level_block(self):
