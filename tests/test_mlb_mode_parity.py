    ...
    return {
        "dates": [{
            ...
            "games": [{
                ...
                "teams": {
                    "away": {"team": {"id": 1, "name": "Away"}, "probablePitcher": {"id": 11, "fullName": "AP"}},
                    "home": {"team": {"id": 2, "name": "Home"}, "probablePitcher": {"id": 22, "fullName": "HP"}},
                },
            }],
        }],
    }
...
def boxscore_payload():
    def players(start):
        return {
            f"ID{start + i}": {
                "person": {"id": start + i, "fullName": str(start + i)},
...
def opener(req, timeout=15):
    ...
    if url == "https://quotes":
        return Resp(raw_quotes())
    if "schedule?" in url:
        return Resp(schedule_payload())
    if "/boxscore" in url:
        return Resp(boxscore_payload())
    if "/teams/" in url and "/stats?" in url:
        parsed = urlparse(url)
        team_id = int(parsed.path.split("/teams/")[1].split("/")[0])
        season = int(parse_qs(parsed.query)["season"][0])
        return Resp(team_history(team_id, season))
    raise AssertionError(url)


def canonical_quotes():
    return [validate_canonical_quote(row) for row in raw_quotes()]
...
def frozen_features(game, quotes):
    ...
    return build_feature_rows_for_quotes(
        games=[game], quotes=quotes, source=source, target_date=TARGET_DATE
    )
    ...
    floors.write_text(json.dumps({
        "truth_gate": {
            ...
            "edge_floors": {
                "MONEYLINE": {
                    "status": "FROZEN",
                    "value_probability_points": 0.50,
                    "method_version": "TEST_ONLY_PARITY_FIXTURE",
                    "evidence": {
                        "evidence_sha256": "1" * 64,
                        "derivation_code_sha256": "2" * 64,
                        "oos_cutoff_utc": "2000-01-01T00:00:00Z",
                    },
                    "frozen": {"frozen_by_commit": "TEST_ONLY_NOT_A_PRODUCTION_PROMOTION"},
                }
            },
        }
    }), encoding="utf-8")
...
def core_snapshot(mock_call):
    ...
    return {
        "games": [asdict(g) for g in kwargs["games"]],
        "feature_rows": kwargs["feature_rows"],
        "quotes": kwargs["quotes"],
        "ingestion_now": kwargs["ingestion_now"],
        "finalization_now": kwargs["finalization_now"],
        "registry_path": kwargs["registry_path"],
        "require_confirmed_lineup": kwargs["require_confirmed_lineup"],
        "edge_floor_config_path": kwargs["edge_floor_config_path"],
        "kelly_multiplier": kwargs["kelly_multiplier"],
    }


def economics(rows):
    fields = (
        "game_id", "market", "entity_id", "line", "side", "american_odds",
        "model_p", "model_input_hash", "distribution_sha256", "readout_sha256",
        "implied_probability", "edge", "ev_per_dollar", "bet_status", "reason",
        "book_key", "sportsbook", "quote_retrieved_at", "offer_id",
    )
...
class MLBModeParityTests(unittest.TestCase):
    def test_manual_hybrid_automatic_normalize_to_identical_core_and_economics(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            common = dict(
                games=[game], quotes=quotes, target_date=TARGET_DATE, now=NOW,
                registry_path=str(registry), require_confirmed_lineup=True,
                edge_floor_config_path=str(floors), kelly_multiplier=KELLY,
            )

            with patch.object(manual_hybrid, "run_unified_card", wraps=run_unified_card) as manual_core:
                manual_results = manual_hybrid.run_manual_hybrid_joint_mlb(
                    feature_rows=frozen, **common
                )
            ...
            with patch.object(manual_hybrid, "run_unified_card", wraps=run_unified_card) as hybrid_core:
                hybrid_results = manual_hybrid.run_manual_hybrid_joint_mlb(
                    feature_rows=None, opener=opener, **common
                )
            ...
            with patch.object(auto_joint, "run_unified_card", wraps=run_unified_card) as auto_core:
                auto_report = auto_joint.run_auto_joint_mlb(
                    quote_url="https://quotes", now=NOW, opener=opener,
                    registry_path=str(registry), require_confirmed_lineup=True,
                    edge_floor_config_path=str(floors), kelly_multiplier=KELLY,
                )
        ...
        expected = economics(manual_results)
        self.assertEqual(expected, economics(hybrid_results))
        self.assertEqual(expected, economics(auto_report.results))
        self.assertTrue(all(row.model_p is not None for row in manual_results))
        self.assertTrue(all(row.implied_probability is not None for row in manual_results))
        self.assertTrue(all(row.edge is not None and row.ev_per_dollar is not None for row in manual_results))
        self.assertTrue(all(row.bet_status == "PASS" for row in manual_results))

    def test_production_policy_fail_closed_decision_is_mode_invariant(self):
        game = live_game()
        quotes = canonical_quotes()
        frozen = frozen_features(game, quotes)
        common = dict(
            games=[game], quotes=quotes, target_date=TARGET_DATE, now=NOW,
            require_confirmed_lineup=True, kelly_multiplier=KELLY,
        )
        manual_results = manual_hybrid.run_manual_hybrid_joint_mlb(feature_rows=frozen, **common)
        hybrid_results = manual_hybrid.run_manual_hybrid_joint_mlb(
            feature_rows=None, opener=opener, **common
        )
        auto_results = auto_joint.run_auto_joint_mlb(
            quote_url="https://quotes", now=NOW, opener=opener,
            require_confirmed_lineup=True, kelly_multiplier=KELLY,
        ).results

        decisions = lambda rows: [(row.bet_status, row.reason) for row in rows]
        self.assertEqual(decisions(manual_results), decisions(hybrid_results))
        self.assertEqual(decisions(hybrid_results), decisions(auto_results))
        self.assertTrue(all(row.bet_status == "BLOCKED" for row in manual_results))
        self.assertTrue(all(row.model_p is None for row in manual_results))
