...
def schedule():
    return {"dates": [{"date": "2026-08-11", "games": [{
        "gamePk": 777, "gameDate": "2026-08-11T23:00:00Z", "officialDate": "2026-08-11",
        "gameNumber": 1, "doubleHeader": "N", "venue": {"id": 3313},
...
def event_odds():
    def outcomes(prefix):
        return [
            {"name": "Over", "description": "Cubs Batter 1", "point": 0.5, "price": -110, "sid": f"{prefix}-over"},
            {"name": "Under", "description": "Cubs Batter 1", "point": 0.5, "price": -110, "sid": f"{prefix}-under"},
        ]
    return {"id": "provider-event", "bookmakers": [{
        "key": "draftkings", "title": "DraftKings", "last_update": "2026-08-11T14:59:00Z",
        "markets": [
            {"key": "batter_hits", "outcomes": outcomes("hits")},
            {"key": "batter_total_bases", "outcomes": outcomes("tb")},
        ],
    }]}
...
def batting_rows():
    ...
    for i in range(25):
        ...
        doubles = 1 if i % 4 == 0 else 0
        triples = 1 if i == 3 else 0
        hits = 2
        out.append({"date": d, "game": {"gamePk": 500+i}, "stat": {
            "hits": hits, "doubles": doubles, "triples": triples, "homeRuns": hr, "plateAppearances": 5,
        }})
    return out
...
def fielding_rows():
    start = date(2026, 4, 1)
    return [{"date": (start + timedelta(days=i)).isoformat(), "game": {"gamePk": 500+i},
             "position": {"abbreviation": "DH" if i % 6 == 0 else "1B"},
             "stat": {"gamesStarted": 1, "innings": "0.0" if i % 6 == 0 else "9.0"}}
            for i in range(25)]


def pitching_rows():
    start = date(2026, 4, 1)
    return [{"date": (start + timedelta(days=7*i)).isoformat(), "game": {"gamePk": 600+i},
             "stat": {"hits": 5, "homeRuns": 1, "battersFaced": 25, "gamesStarted": 1}}
            for i in range(5)]
...
class FullAutoNativeAcceptance(unittest.TestCase):
    ...
    def test_hits_and_total_bases_run_without_manual_quote_or_feature_snapshots(self):
        report = run_auto_mlb_native_odds(
            odds_api_key="synthetic-key",
            feature_url=None,
            projected_lineups_url=None,
            now=NOW,
            opener=self.opener,
        )
        self.assertEqual(report.slate_date_ct, "2026-08-11")
        self.assertEqual(len(report.results), 4)
        self.assertEqual({r.market for r in report.results}, {"HITS", "TOTAL_BASES"})
        self.assertEqual({r.entity_id for r in report.results}, {"100"})
        self.assertEqual({r.side for r in report.results}, {"OVER", "UNDER"})
        self.assertTrue(all(r.bet_status == "BLOCKED" for r in report.results))
        # Blocking must be deployment truth, not missing acquisition inputs.
        self.assertTrue(all("feature" not in r.reason.lower() for r in report.results))
        self.assertTrue(all("quote" not in r.reason.lower() for r in report.results))
