...
class CFBForwardCLVCaptureTests(unittest.TestCase):
    ...
    def test_policy_is_fail_closed_and_first_slate_is_sep19(self):
        self.assertEqual(self.policy["version"], "1.0.3")
        ...
        self.assertEqual(close["boundary_basis"], "DUAL_SOURCE_PRE_START_PLUS_STABILIZED_POSTHOC_FIRST_PLAY_ATTESTATION")
        self.assertEqual(close["actual_start_attestation"]["stabilization_delay_hours"], 12)
    ...
    def test_status_unknown_inprogress_and_postponed_fail_closed(self):
        ...
        self.assertTrue(mod.status_is_pre(pre, received, self.policy, at=received)[0])
        self.assertFalse(mod.status_is_pre({"status_state": "in", "status_name": "STATUS_IN_PROGRESS", "completed": False}, received, self.policy, at=received)[0])
        ok, reason = mod.status_is_pre({"status_state": "pre", "status_name": "STATUS_POSTPONED", "completed": False}, received, self.policy, at=received)
        self.assertFalse(ok)
    ...
    def test_first_play_attestation_policy_is_uncertainty_aware(self):
        att = self.policy["capture_schedule"]["close"]["actual_start_attestation"]
        self.assertTrue(att["timestamp_precision_required"])
        self.assertEqual(att["max_acceptable_timestamp_uncertainty_seconds"], 30)
        self.assertEqual(att["safety_margin_seconds"], 30)
        self.assertIn("timestamp_uncertainty_seconds", att["validity_rule"])
    ...
    def test_small_cluster_reference_uses_bootstrap_and_excludes_postseason(self):
        ...
        self.assertEqual(se["small_sample_reference"], "WILD_CLUSTER_BOOTSTRAP_T_WEBB")
        self.assertEqual(se["bootstrap_repetitions"], 9999)
        self.assertEqual(se["bootstrap_weights"], "WEBB_SIX_POINT")
        regime = req["eligible_cluster_regime"]
        self.assertEqual(regime["primary"], "FBS_REGULAR_SEASON_ONLY")
        self.assertIn("POSTSEASON_DIAGNOSTIC_ONLY", regime["bowls"])
        self.assertIn("POSTSEASON_DIAGNOSTIC_ONLY", regime["playoff"])
