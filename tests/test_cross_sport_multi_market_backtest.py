...
def test_rejects_post_decision_feature():
    row = _row(season=2025, i=1, p=0.55, y=1)
    row["feature_asof_ts"] = "2025-09-01T11:30:00+00:00"
    with pytest.raises(ResearchBacktestError, match="FEATURE_AFTER_DECISION"):
        canonicalize_rows([row])


def test_rejects_mixed_market_batch():
    a = _row(season=2025, i=1, p=0.55, y=1)
    b = _row(season=2025, i=2, p=0.45, y=0)
    b["market"] = "TOTAL"
    with pytest.raises(ResearchBacktestError, match="MIXED_SPORT_OR_MARKET"):
        canonicalize_rows([a, b])


def test_summary_and_roi_are_settlement_aware():
    rows = [
    ...
    assert summary["n_binary"] == 2
    assert summary["n_push_or_void"] == 1
    assert summary["bet_n"] == 3
    assert summary["bet_profit_units"] == pytest.approx(0.82)
    assert summary["bet_roi"] == pytest.approx(0.82 / 3.0)


def test_walkforward_never_uses_future_season():
    rows = []
    for season in range(2020, 2026):
        for i in range(2):
            rows.append(_row(season=season, i=i, p=0.55, y=i % 2))
    folds = completed_season_walkforward(rows, min_train_seasons=3)
    assert folds[0]["test_season"] == 2023
    for fold in folds:
        assert all(season < fold["test_season"] for season in fold["train_seasons"])


def test_paired_bootstrap_is_deterministic_and_negative_is_better():
    rows = []
    # Candidate probabilities match deterministic outcomes more closely than 0.50.
    ...
    assert first == second
    assert first["brier_delta_model_minus_benchmark"] < 0
    assert first["log_loss_delta_model_minus_benchmark"] < 0
    assert first["brier_delta_ci95"][1] < 0
    assert first["log_loss_delta_ci95"][1] < 0
