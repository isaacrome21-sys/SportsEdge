...
from sportsedge.sports.nfl.m2_v2f_candidate import (
    derive_nfl_m2_v2f_score_distribution,
    fit_nfl_m2_v2f_candidate,
)


class NFLM2V2FNativeScoreTests(unittest.TestCase):
    ...
    def test_preregistered_defaults_are_fixed(self):
        model = fit_nfl_m2_v2f_candidate(self._rows())
        self.assertEqual(model.mean_model.ridge_alpha, 10.0)
        self.assertEqual(model.home_score_kernel_scale, 1.0)
        self.assertEqual(model.away_score_kernel_scale, 1.0)
        self.assertEqual(model.min_coordinate_train_seasons, 1)

    def test_support_coordinates_are_strictly_prior_season(self):
        model = fit_nfl_m2_v2f_candidate(self._rows(), ridge_alpha=2.0)
    ...
    def test_support_season_outcome_cannot_change_its_own_coordinates(self):
        rows = self._rows()
        baseline = fit_nfl_m2_v2f_candidate(rows, ridge_alpha=2.0)
        changed_rows = copy.deepcopy(rows)
        for row in changed_rows:
            if row["season"] == 2021:
                row["home_score"] = 60
                row["away_score"] = 3
        changed = fit_nfl_m2_v2f_candidate(changed_rows, ridge_alpha=2.0)
        ...
        self.assertEqual(
            [(p.predicted_home_score, p.predicted_away_score, p.coordinate_train_seasons) for p in base_points],
            [(p.predicted_home_score, p.predicted_away_score, p.coordinate_train_seasons) for p in changed_points],
        )
        self.assertNotEqual(
    ...
    def test_root_market_fields_cannot_change_candidate(self):
        ...
        changed = fit_nfl_m2_v2f_candidate(contaminated, ridge_alpha=2.0)
        self.assertEqual(baseline, changed)

    def test_distribution_is_normalized_observed_integer_support(self):
        rows = self._rows()
        model = fit_nfl_m2_v2f_candidate(rows, ridge_alpha=2.0)
        distribution = derive_nfl_m2_v2f_score_distribution(model, rows[-1])
        observed = {
