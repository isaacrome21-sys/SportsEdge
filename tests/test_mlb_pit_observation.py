...
def _row(**changes):
    row = {
        ...
        "source_evidence_class": "SYNTHETIC_CONTRACT_TEST",
...
class MLBPITObservationTests(unittest.TestCase):
    ...
    def test_pregame_quote_and_history_are_required(self):
        ...
        with self.assertRaisesRegex(MLBPITObservationError, "history_asof_ts must be before"):
            normalize_pit_observation(_row(history_asof_ts="2026-08-24T00:11:00+00:00"))

    def test_settlement_eligible_requires_scored_outcome_and_rule_hash(self):
    ...
    def test_ambiguous_or_void_rows_cannot_carry_scored_outcome(self):
        for state in ("AMBIGUOUS_SETTLEMENT", "VOID", "BOOK_SETTLEMENT_UNVALIDATED"):
            ...
            with self.assertRaisesRegex(MLBPITObservationError, "must not carry a scored outcome"):
                normalize_pit_observation(_row(settlement_state=state, settled_outcome="LOSS"))
    ...
    def test_synthetic_rows_never_become_historical_pit(self):
        report = analyze_pit_observations([_row()])
    ...
    def test_fully_proven_live_row_can_be_labeled_historical_pit(self):
        report = analyze_pit_observations(
            [_row(source_evidence_class="LIVE_PROVIDER_QUOTE_ARCHIVE")]
        )
