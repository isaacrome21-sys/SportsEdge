...
from sportsedge.dfs.scoring import football_expected_dk_points, mlb_hitter_expected_dk_points, projection_from_samples
...
def test_dk_lobby_and_draftables_normalization() -> None:
    lobby = {"DraftGroups": [{"DraftGroupId": 123, "StartDate": "2026-09-18T23:30:00Z", "GameCount": 5, "GameTypeId": 1}], "Contests": [{"id": 7, "dg": 123, "n": "CFB Main", "po": 150000}]}
    draftables = {"draftables": [{"draftableId": 999, "playerDkId": 44, "displayName": "Quarter Back", "position": "QB", "salary": 8000, "teamAbbreviation": "AAA", "competition": {"competitionId": 2, "name": "AAA @ BBB", "startTime": "2026-09-18T23:30:00Z"}, "draftStatAttributes": [{"id": 90, "value": 22.5}]}]}
...
def test_nfl_optimizer_builds_qb_stack_and_valid_roster() -> None:
    rows = [
        p("qb1", "QB A", "A", "B", "QB", 6500, 23), p("qb2", "QB B", "B", "A", "QB", 6100, 19),
        p("rb1", "RB A", "A", "B", "RB", 7000, 19), p("rb2", "RB B", "B", "A", "RB", 6500, 18), p("rb3", "RB C", "C", "D", "RB", 5800, 16),
        p("wr1", "WR A1", "A", "B", "WR", 7200, 21), p("wr2", "WR A2", "A", "B", "WR", 5200, 15), p("wr3", "WR B1", "B", "A", "WR", 6000, 17), p("wr4", "WR C1", "C", "D", "WR", 4800, 14),
        p("te1", "TE A", "A", "B", "TE", 4200, 12), p("te2", "TE B", "B", "A", "TE", 3800, 10),
        p("dst1", "DST C", "C", "D", "DST", 3000, 8), p("dst2", "DST D", "D", "C", "DST", 2800, 7),
    ]
    ...
    qb = [e.player for e in lineup.entries if e.slot == "QB"][0]
...
def test_mlb_optimizer_avoids_batters_against_selected_pitchers() -> None:
    rows = [
        p("p1", "P A", "A", "B", "P", 9000, 23), p("p2", "P C", "C", "D", "P", 8500, 21), p("p3", "P E", "E", "F", "P", 7000, 16),
        p("c1", "C A", "A", "B", "C", 3800, 9), p("c2", "C B", "B", "A", "C", 3600, 13), p("1b", "1B A", "A", "B", "1B", 4300, 12),
        p("2b", "2B C", "C", "D", "2B", 4100, 11), p("3b", "3B E", "E", "F", "3B", 4000, 10), p("ss", "SS A", "A", "B", "SS", 4200, 11),
        p("of1", "OF A", "A", "B", "OF", 4500, 13), p("of2", "OF C", "C", "D", "OF", 4400, 12), p("of3", "OF E", "E", "F", "OF", 3900, 10), p("of4", "OF G", "G", "H", "OF", 3500, 9),
    ]
    players = [x[0] for x in rows]
    projections = {x[0].player_id: x[1] for x in rows}
...
def test_cfb_optimizer_supports_superflex_and_qb_stacks() -> None:
    rows = [
        p("qa", "QB A", "A", "B", "QB", 9000, 28), p("qb", "QB B", "B", "A", "QB", 8200, 25), p("qc", "QB C", "C", "D", "QB", 7000, 20),
        p("ra", "RB A", "A", "B", "RB", 6500, 18), p("rb", "RB B", "B", "A", "RB", 6100, 17), p("rc", "RB C", "C", "D", "RB", 5200, 14),
        p("wa", "WR A", "A", "B", "WR", 6200, 19), p("wb", "WR B", "B", "A", "WR", 5700, 17), p("wc", "WR C", "C", "D", "WR", 4800, 14), p("wd", "WR D", "D", "C", "WR", 4200, 12), p("we", "WR A2", "A", "B", "WR", 3900, 11), p("wf", "WR B2", "B", "A", "WR", 3600, 10), p("ta", "TE A", "A", "B", "TE", 3300, 9),
    ]
    players = [x[0] for x in rows]
    projections = {x[0].player_id: x[1] for x in rows}
    ...
    for qb in [e.player for e in lineup.entries if "QB" in e.player.positions]:
        assert any(e.player.player_id != qb.player_id and e.player.team == qb.team and set(e.player.positions) & {"WR", "TE"} for e in lineup.entries)


def test_complete_distribution_converts_to_dfs_projection_and_partial_fails_closed() -> None:
    player, _ = p("qb", "QB", "A", "B", "QB", 7000, 20)
    complete = [{"passing_yards": 300, "passing_tds": 2, "interceptions": 1, "rushing_yards": 20, "rushing_tds": 0} for _ in range(1000)]
    proj = projection_from_samples(player, "NFL", complete, source="TEST_PATHS")
    assert proj.mean > 0
    assert proj.components["simulation_paths"] == 1000
    partial = [{"passing_yards": 300} for _ in range(1000)]
    try:
        projection_from_samples(player, "NFL", partial, source="BAD_PATHS")
    except ValueError as exc:
        assert "DFS_SAMPLE_COMPONENTS_MISSING" in str(exc)
    else:
        raise AssertionError("partial distribution should fail closed")


def test_dk_salary_csv_fallback(tmp_path) -> None:
    path = tmp_path / "DKSalaries.csv"
    path.write_text(
        "Position,Name,ID,Roster Position,Salary,Game Info,TeamAbbrev,AvgPointsPerGame\n"
        "QB,Quarter Back,44,QB,8000,AAA@BBB 09/18/2026 07:30PM ET,AAA,22.5\n",
        encoding="utf-8",
    )
    players = DraftKingsClient(getter=lambda _: {}).load_salary_csv(path)
    assert players[0].player_id == "44"
    assert players[0].salary == 8000
    assert players[0].opponent == "BBB"
