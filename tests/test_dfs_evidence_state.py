...
def test_downstream_fields_alone_do_not_make_mlb_pitcher_lane_ready() -> None:
    state = mlb_pitcher_upstream_state(
        ...
        hook_conditioned_on_pitch_count=False,
        hook_conditioned_on_runs_allowed=False,
        bullpen_remainder_routed=False,
        hit_conservation_validated=False,
    ...
    assert "HOOK_PITCH_COUNT_STATE" in state.reason
    assert "HOOK_RUNS_ALLOWED_STATE" in state.reason
    assert "BULLPEN_REMAINDER_ROUTING" in state.reason
    assert "TEAM_HIT_CONSERVATION" in state.reason
    assert "POST_EXIT_FULL_GAME_CONTINUATION" in state.reason
    assert state.evidence_accruing is False


def test_mlb_pitcher_lane_requires_structure_conservation_and_all_exit_fields() -> None:
    state = mlb_pitcher_upstream_state(
        _REQUIRED_PITCHER_FIELDS,
    ...
    assert state.reason == "UPSTREAM_ENDOGENOUS_HOOK_BULLPEN_AND_FINAL_STATE_AVAILABLE"
    assert state.evidence_accruing is True
