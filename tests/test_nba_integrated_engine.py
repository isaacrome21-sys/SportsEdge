...
def _bundle():
    rows = (_row("g1", 112, 100), _row("g2", 108, 106))
    pace = fit_pace_efficiency(rows)
...
def test_supported_markets_bind_from_shared_paths():
    card = bind_integrated_card(
        [
            ...
            _q("SPREAD", "HOME", -3.5),
            _q("TOTAL", "OVER", 220.5),
            _q("HOME_TEAM_TOTAL", "OVER", 112.5),
            _q("FIRST_HALF_TOTAL", "UNDER", 110.5),
            _q("PLAYER_POINTS", "p1", 23.5),
            _q("PLAYER_PRA", "p1", 36.5),
        ],
...
def test_unsupported_markets_remain_explicitly_unsupported():
    card = bind_integrated_card(
        [_q("FIRST_BASKET", "p1"), _q("DOUBLE_DOUBLE", "p1", 0.5)],
        _bundle(), as_of=AS_OF, model_quality=1.0, context_quality=1.0,
    )
    assert card.rows == ()
