import tempfile
import unittest
from pathlib import Path

from sportsedge.core.validation.evidence_receipt import (
    EvidenceReceiptError,
    build_evidence_receipt,
    verify_evidence_receipt,
)


class EvidenceReceiptTests(unittest.TestCase):
    def test_receipt_is_order_independent_and_mutation_sensitive(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            a = root / "a.json"
            b = root / "b.json"
            a.write_text('{"a":1}\n', encoding="utf-8")
            b.write_text('{"b":2}\n', encoding="utf-8")

            r1 = build_evidence_receipt(
                [("inputs/a.json", a), ("inputs/b.json", b)],
                sport="mlb",
                purpose="replay",
                metadata={"target_direction": "EARLY_ONLY_AT_OR_BEFORE_TARGET"},
            )
            r2 = build_evidence_receipt(
                [("inputs/b.json", b), ("inputs/a.json", a)],
                sport="mlb",
                purpose="replay",
                metadata={"target_direction": "EARLY_ONLY_AT_OR_BEFORE_TARGET"},
            )
            self.assertEqual(r1["receipt_sha256"], r2["receipt_sha256"])
            self.assertFalse(r1["promotion_authority"])
            self.assertEqual(
                verify_evidence_receipt(
                    r1, {"inputs/a.json": a, "inputs/b.json": b}
                )["status"],
                "PASS",
            )

            a.write_text('{"a":9}\n', encoding="utf-8")
            checked = verify_evidence_receipt(
                r1, {"inputs/a.json": a, "inputs/b.json": b}
            )
            self.assertEqual(checked["status"], "FAIL")
            self.assertIn("SHA256_MISMATCH:inputs/a.json", checked["differences"])

    def test_receipt_rejects_unsafe_or_duplicate_logical_paths(self):
        with tempfile.TemporaryDirectory() as td:
            file_path = Path(td) / "x"
            file_path.write_text("x", encoding="utf-8")
            with self.assertRaises(EvidenceReceiptError):
                build_evidence_receipt(
                    [("../escape", file_path)], sport="cfb", purpose="audit"
                )
            with self.assertRaises(EvidenceReceiptError):
                build_evidence_receipt(
                    [("x", file_path), ("x", file_path)],
                    sport="cfb",
                    purpose="audit",
                )


if __name__ == "__main__":
    unittest.main()
