...
class TestFirstPlayAttestation(unittest.TestCase):
    ...
    def test_blocks_before_12h_stabilization(self):
        out = self.evaluate(
            self.first - timedelta(minutes=2),
            sealed_at_ts=self.final + timedelta(hours=11, minutes=59),
        )
...
class TestClusterRegime(unittest.TestCase):
    ...
    def test_conference_championship_is_diagnostic_only(self):
        self.assertEqual(classify_cluster_regime(notes="conference championship"), "POSTSEASON_DIAGNOSTIC_ONLY")
...
class TestWildClusterBootstrap(unittest.TestCase):
    ...
    def test_bootstrap_is_deterministic(self):
        ...
        self.assertEqual(a, b)
        self.assertEqual(a["clusters"], 12)
        self.assertTrue(0 < a["two_sided_p"] <= 1)

    def test_postseason_rows_cannot_supply_cluster_floor(self):
        rows = []
        for i in range(1, 12):
            rows.append({"season_type": "regular season", "slate_date_ct": f"2026-09-{i:02d}", "clv_pp": 2.0})
        ...
        self.assertFalse(out["inference_gate_pass"])
        self.assertEqual(out["eligible_regular_season_clusters"], 11)
    ...
    def test_bootstrap_cannot_rescue_failed_mean(self):
        rows = [
            {"season_type": "regular season", "slate_date_ct": f"2026-09-{i:02d}", "clv_pp": 0.1}
            for i in range(1, 13)
        ]
        out = evaluate_promotion_inference(rows, policy_sha256="abc", market_ledger="CFB_TOTAL", repetitions=99)
        self.assertFalse(out["inference_gate_pass"])
        self.assertIn("MEAN_CLV_GATE_FAILED", out["blockers"])
        self.assertIsNone(out["bootstrap_two_sided_p"])
