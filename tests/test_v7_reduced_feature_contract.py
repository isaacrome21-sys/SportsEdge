...
def _full_payload(contract: dict, seed: int) -> dict:
    payload = {"feature_contract_version": "mlb_v7_combined_feature_contract_v1", "feature_contract_sha256": V7_COMBINED_FEATURE_CONTRACT_SHA256, "feature_as_of_utc": f"2024-01-{(seed % 20) + 1:02d}T12:00:00+00:00", "baseball": {}, "context": {}, "ignored_metadata": "discard-me"}
...
def _contract():
    return load_reduced_feature_contract(CONTRACT_PATH, policy_path=POLICY_PATH, requirements_path=REQUIREMENTS_PATH)
...
def test_trainer_stamps_reduced_contract_and_only_twelve_coefficients() -> None:
    ...
    start = date(2023, 1, 1)
    for idx in range(10):
        rows.append({"game_date": (start + timedelta(days=idx)).isoformat(), "outcome": idx % 2, "feature_payload": project_full_payload_to_reduced(_full_payload(contract, idx), contract=contract)})
    for idx in range(2):
        rows.append({"game_date": date(2024, 1, idx + 1).isoformat(), "outcome": idx, "feature_payload": project_full_payload_to_reduced(_full_payload(contract, 20 + idx), contract=contract)})
    artifact, report = train_chronological_candidate(rows, model_name="V7_REDUCED_TEST", train_end="2023-12-31", calibration_start="2024-01-01", calibration_end="2024-12-31", holdout_start="2025-01-01", feature_paths=contract["feature_paths"], feature_contract_sha256=contract["feature_contract_sha256"], iterations=5, learning_rate=0.05)
...
def test_trainer_rejects_reduced_paths_with_full_contract_hash() -> None:
    contract = _contract()
    rows = [{"game_date": f"2023-01-{idx + 1:02d}", "outcome": idx % 2, "feature_payload": project_full_payload_to_reduced(_full_payload(contract, idx), contract=contract)} for idx in range(10)]
    rows.extend([{"game_date": "2024-01-01", "outcome": 0, "feature_payload": project_full_payload_to_reduced(_full_payload(contract, 30), contract=contract)}, {"game_date": "2024-01-02", "outcome": 1, "feature_payload": project_full_payload_to_reduced(_full_payload(contract, 31), contract=contract)}])
    with pytest.raises(V7TrainingError, match="contract hash mismatch"):
        train_chronological_candidate(rows, model_name="BAD_BINDING", train_end="2023-12-31", calibration_start="2024-01-01", calibration_end="2024-12-31", holdout_start="2025-01-01", feature_paths=contract["feature_paths"], feature_contract_sha256=V7_COMBINED_FEATURE_CONTRACT_SHA256, iterations=5)
