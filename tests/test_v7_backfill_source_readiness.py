    ...
    for group in req["groups"].values():
        for source_class, source_req in group["sources"].items():
            ...
            manifest.write_text(json.dumps({
                ...
                "fields": source_req["required_fields"],
                "semantics": source_req["required_semantics"],
                "evidence_files": [{
                    "path": evidence.relative_to(source_root).as_posix(),
...
def test_missing_source_root_fails_hard(tmp_path: Path) -> None:
    with pytest.raises(SourceReadinessError, match="SOURCE_ROOT_MISSING"):
        _audit(tmp_path / "missing")
...
def test_policy_requirement_feature_set_mismatch_fails_hard(tmp_path: Path) -> None:
    ...
    source_root.mkdir()
    req = json.loads(REQUIREMENTS.read_text())
    req["groups"]["bullpen"]["feature_paths"] = req["groups"]["bullpen"]["feature_paths"][:-1]
    altered = tmp_path / REQUIREMENTS.name
    altered.write_text(json.dumps(req))

    with pytest.raises(SourceReadinessError, match="POLICY_REQUIREMENTS_FEATURE_SET_MISMATCH"):
...
def test_missing_final_before_decision_semantics_blocks_bullpen(tmp_path: Path) -> None:
    ...
    manifest["semantics"].pop("final_before_decision")
...
def test_tampered_evidence_file_blocks_source(tmp_path: Path) -> None:
    ...
    source_root.mkdir()
    _write_complete_attestations(source_root)
    evidence = source_root / "raw" / "PARK_RAW.json"
    evidence.write_text("tampered\n")

    report = _audit(source_root)

    park = report["groups"]["park"]["sources"]["PARK_RAW"]
