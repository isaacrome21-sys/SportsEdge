...
from sportsedge.sports.nfl.m2_v2d_validation import (
    build_nfl_m2_v2d_candidate_evidence,
    build_nfl_m2_v2d_raw_evaluations,
)


class NFLM2V2DValidationTests(unittest.TestCase):
    ...
    def test_outer_heldout_outcome_does_not_change_same_fold_probabilities(self):
        rows = self._rows()
        baseline = build_nfl_m2_v2d_raw_evaluations(rows, min_train_seasons=2, ridge_alpha=2.0)
        ...
        for row in changed_rows:
            if row["season"] == first_test:
                row["home_score"] = 70
                row["away_score"] = 0
        changed = build_nfl_m2_v2d_raw_evaluations(changed_rows, min_train_seasons=2, ridge_alpha=2.0)
    ...
    def test_evidence_is_diagnostic_only(self):
        ...
        self.assertEqual(set(evidence["candidate_historical_evidence"]), {"spread", "total"})
