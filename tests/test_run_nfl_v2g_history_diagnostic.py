...
def _cmd(schedule, pbp, out, manifest, code_sha=CODE_SHA):
    return [
        sys.executable,
        "scripts/run_nfl_v2g_history_diagnostic.py",
...
def test_runner_fails_closed_when_td_team_missing(tmp_path):
    ...
    proc = subprocess.run(_cmd(schedule, pbp, out, manifest), capture_output=True, text=True)
    assert proc.returncode != 0
    assert "NFL_V2G_REQUIRED_SOURCE_FIELDS_MISSING" in (proc.stderr + proc.stdout)
    assert not manifest.exists()


def test_runner_fails_closed_on_invalid_code_sha(tmp_path):
    schedule = tmp_path / "schedule.csv"
    pbp = tmp_path / "pbp.csv"
    out = tmp_path / "events.json"
    manifest = tmp_path / "manifest.json"
    ...
    _write_csv(pbp, fields, [{"game_id":"2025_01_A_B","drive":1,"posteam":"A","touchdown":0,"td_team":"","play_type":"run","field_goal_result":""}])
    proc = subprocess.run(_cmd(schedule, pbp, out, manifest, code_sha="not-a-sha"), capture_output=True, text=True)
    assert proc.returncode != 0
    assert "NFL_V2G_INVALID_CODE_SHA" in (proc.stderr + proc.stdout)
    assert not manifest.exists()
