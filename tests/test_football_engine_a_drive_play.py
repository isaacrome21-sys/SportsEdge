...
class FootballEngineADrivePlayTests(unittest.TestCase):
    def test_hand_built_play_path_reconciles_to_shared_scoring_path(self):
        ...
        path = FootballPlayPath(
            ...
            plays=(
                ...
                PlayEvent(
                    ...
                    possession="HOME", score_before_home=0, score_before_away=0,
                    score_after_home=6, score_after_away=0, down=2, distance=5,
                    yardline_100=5, play_type="PASS", yards=5, points=6,
                    score_type="TOUCHDOWN_CANDIDATE", pass_complete=True,
                ),
                PlayEvent(
                    drive_id=2, play_id=3, quarter=1, clock_seconds_remaining=700,
                    possession="AWAY", score_before_home=6, score_before_away=0,
                    score_after_home=6, score_after_away=0, down=4, distance=7,
                    yardline_100=30, play_type="FIELD_GOAL", yards=0, points=0,
                    score_type="FIELD_GOAL_ATTEMPT_CANDIDATE", kick_distance=47,
                ),
            ),
        )
        ...
        self.assertEqual(sum(event.points for event in score_path.events), 6)
    ...
    def test_score_chain_break_fails_closed(self):
        ...
        plays = (
            PlayEvent(
                drive_id=1, play_id=1, quarter=1, clock_seconds_remaining=850,
                possession="HOME", score_before_home=0, score_before_away=0,
                score_after_home=6, score_after_away=0, down=1, distance=10,
                yardline_100=5, play_type="RUSH", yards=5, points=6,
    ...
    def test_generated_scores_only_change_on_tagged_scoring_plays(self):
        ...
        self.assertTrue(all(play.points == 6 for play in scoring))
        self.assertTrue(all(play.score_type == "TOUCHDOWN_CANDIDATE" for play in scoring))
        self.assertTrue(all(
            (play.score_after_home - play.score_before_home)
            + (play.score_after_away - play.score_before_away) == play.points
            for play in scoring
        ))
        field_goals = [play for path in paths for play in path.plays if play.play_type == "FIELD_GOAL"]
        self.assertTrue(field_goals)
        self.assertTrue(all(play.points == 0 for play in field_goals))
