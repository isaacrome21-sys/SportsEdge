...
def _reference_arrays(kwargs,n):
    ...
    return np.array([p.margin for p in ps]),np.array([p.total for p in ps]),np.array([len(p.possessions) for p in ps])
...
def _assert_equivalent(kwargs,n=200_000):
    rm,rt,rp=_reference_arrays(kwargs,n)
    ...
    for k in (3,7):
        ...
        if d>.003:
            ...
            rm2,_,_=_reference_arrays({**kwargs,"seed":kwargs["seed"]+10000},1_000_000)
...
def test_vector_reference_equal_strength_distribution():
    _assert_equivalent(dict(seed=4101))
