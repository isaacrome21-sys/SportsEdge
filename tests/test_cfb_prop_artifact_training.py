...
FIELDS = [
    ...
    "start.yardsToEndzone", "fg_attempt", "fg_made",
    "pointAfterAttempt.text", "pointAfterAttempt.abbreviation", "pointAfterAttempt.value",
    "gameSpread", "overUnder", "homeTeamSpread",
]


def _row(*, game: str, team_id: str, team: str, opponent_id: str, opponent: str,
         clock: int, kind: str = "rush", yards: int = 5, success: bool = True,
         completion: bool = False, down: int = 1, yte: int = 70,
         fg_attempt: bool = False, fg_made: bool = False,
         pat_text: str = "", pat_abbr: str = "", pat_value: str = "",
         spread: str = "-3.5", total: str = "55.5") -> dict[str, str]:
    return {
        "game_id": game, "season": "2025", "seasonType": "2", "status_type_completed": "TRUE",
        "homeTeamId": team_id, "awayTeamId": opponent_id,
        "homeTeamName": team, "awayTeamName": opponent,
        ...
        "int": "FALSE", "fumble_lost": "FALSE", "penalty_no_play": "FALSE",
        "start.adj_TimeSecsRem": str(clock), "down": str(down),
        "start.yardsToEndzone": str(yte), "fg_attempt": "TRUE" if fg_attempt else "FALSE",
        "fg_made": "TRUE" if fg_made else "FALSE",
        "pointAfterAttempt.text": pat_text, "pointAfterAttempt.abbreviation": pat_abbr,
        "pointAfterAttempt.value": pat_value,
...
def _fixture(path: Path, *, market_variant: bool = False) -> None:
    ...
    for idx, (team_id, team, opp_id, opp) in enumerate((
        ...
        # not the type of try. Include a successful XP and one successful/one
        # failed 2PT across the two teams so the semantic sanity bands are real.
        rows.append(_row(
            game=game, team_id=team_id, team=team, opponent_id=opp_id, opponent=opp,
            clock=3300, kind="admin", pat_text="Extra Point Good", pat_abbr="XP", pat_value="1",
        ))
        rows.append(_row(
            game=game, team_id=team_id, team=team, opponent_id=opp_id, opponent=opp,
            clock=3290, kind="admin", pat_text="Two-Point Conversion " + ("Good" if idx == 0 else "Failed"),
            pat_abbr="2PT", pat_value="2" if idx == 0 else "0",
        ))
...
class CFBPropArtifactTrainingTests(unittest.TestCase):
    def _fit(self, path: Path):
        with patch("sportsedge.sports.cfb.prop_artifact_training.MIN_TEAM_COUNT", 2), patch(
            ...
            return fit_cfb_prop_artifact(
                [path], code_git_sha="a" * 40, artifact_version="CFB_PROP_TEST_V1", seasons=[2025]
            )

    def test_fits_market_blind_cfb_artifact_with_plausible_special_teams(self):
        with TemporaryDirectory() as temp:
            path = Path(temp) / "play_by_play_2025.csv"; _fixture(path)
    ...
    def test_sportsbook_columns_cannot_change_fitted_profiles(self):
        with TemporaryDirectory() as temp:
            first = Path(temp) / "a.csv"; second = Path(temp) / "b.csv"
            _fixture(first); _fixture(second, market_variant=True)
            artifact_a, _ = self._fit(first); artifact_b, _ = self._fit(second)
    ...
    def test_missed_try_value_zero_still_counts_as_attempt(self):
        with TemporaryDirectory() as temp:
            path = Path(temp) / "play_by_play_2025.csv"; _fixture(path)
            _, diagnostics = self._fit(path)
        self.assertEqual(diagnostics["league_special_teams"]["two_point_attempts"], 2)
        self.assertEqual(diagnostics["league_special_teams"]["two_point_success_rate"], 0.5)
    ...
    def test_penalty_no_play_and_nonregular_rows_do_not_enter_fit(self):
        with TemporaryDirectory() as temp:
            ...
            with path.open("a", encoding="utf-8", newline="") as handle:
                writer = csv.DictWriter(handle, fieldnames=FIELDS); writer.writerow(bad); writer.writerow(post)
