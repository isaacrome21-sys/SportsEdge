...
class NFLProductionValidationTests(unittest.TestCase):
    def _feature(self, season, game, side):
        ...
        start = f"{season}-09-{10 + game:02d}T17:00:00+00:00"
        asof = f"{season}-09-{9 + game:02d}T17:00:00+00:00"
        return build_nfl_m2_features({
            "off_epa": 0.05 + strength,
            "def_epa": -0.02 - 0.3 * strength,
            "pass_epa": 0.08 + strength,
            "rush_epa": 0.01 + 0.3 * strength,
            "opp_off_epa": 0.03 - 0.1 * strength,
            "opp_def_epa": -0.01 + 0.1 * strength,
            "pressure_for": 0.28 + 0.02 * sign,
            "pressure_allowed": 0.26 - 0.01 * sign,
            "success_rate": 0.44 + 0.05 * strength,
            "explosive_rate": 0.10 + 0.02 * strength,
            "rest_diff_days": sign,
            "travel_miles": 0.0 if side == "home" else 700.0,
            "timezone_crossings": 0.0 if side == "home" else 1.0,
            "short_week": 0.0,
            "bye_week": 0.0,
            "wind_mph": 8.0,
            "roof_closed": 0.0,
            "qb_id": f"{side}_{season}_{game}",
            "qb_adjustment": 0.8 * strength,
            "prior_efficiency": 0.4 * strength,
            "prior_weight": 0.5,
    ...
    def _rows(self):
        ...
        for season in range(2018, 2026):
            for game in range(1, 7):
                home = self._feature(season, game, "home")
                away = self._feature(season, game, "away")
    ...
    def test_mutating_heldout_results_does_not_change_same_fold_model_probabilities(self):
        rows = self._rows()
        baseline = build_production_nfl_raw_evaluations(rows, min_train_seasons=2, ridge_alpha=1.0)
        ...
        self.assertEqual(
            [(r["game_id"], r["m2_home_cover_prob"], r["m2_over_prob"]) for r in left],
    ...
    def test_evidence_payload_is_exact_production_identity_and_reports_brier_and_log_loss(self):
        payload = build_production_nfl_validation_evidence(
            self._rows(),
            source_uri="frozen://nfl-m2-source-manifest",
            source_sha256="a" * 64,
            min_train_seasons=2,
        ...
        self.assertTrue(payload["folds"])
        self.assertIn("spread", payload["promotion_evidence"])
        self.assertIn("total", payload["promotion_evidence"])
        for fold in payload["folds"]:
            self.assertIn("m1_log_loss", fold)
            self.assertIn("m2_log_loss", fold)
            self.assertIn("m1_brier", fold)
            self.assertIn("m2_brier", fold)

    def test_missing_market_prices_reduce_m1_coverage_not_m2_population(self):
        rows = self._rows()
        rows[-1]["home_spread_odds"] = None
        rows[-1]["away_spread_odds"] = None
        ...
        self.assertIsNone(last["m1_home_cover_prob"])
